...
- **VPN-Texte: „ThinVPN" und „VPN-Instanz" sind jetzt auseinandergehalten** — Derselbe Begriff bezeichnete bisher zwei verschiedene Dinge: „ThinVPN-Server" meinte mal den VPN-Dienst auf dem ThinForge-Server, mal die entfernte VPN-Instanz, bei der er sich anmeldet. Der Dienst heißt weiterhin **ThinVPN**; die Gegenstelle heißt jetzt durchgehend **VPN-Instanz** beziehungsweise **VPN-Verwaltung**. Ein Satz wie „ThinForge-Server beim ThinVPN-Server anmelden" kommt damit nicht mehr vor. Zwei ältere Unstimmigkeiten fielen dabei mit auf und sind behoben: die Startseite ohne VPN und die Statusleiste nannten denselben Zustand unterschiedlich, und zwei Hinweise zum Lösen sprachen vom „NetBird-Dienst" statt vom ThinVPN-Dienst. *(Verwaltungsoberfläche)*

- **Der Server wird beim Wiederanmelden aufgenommen, statt ersetzt** — Meldete sich der Server nach einem Verbindungsverlust neu an, legte ThinForge dafür immer einen neuen Eintrag auf der VPN-Instanz an; der bisherige blieb dort liegen, mit gültigem Zugang, aber ohne Zuordnung zu einem Gerät. Das betraf vor allem den Weg über „Übernehmen". ThinForge versucht jetzt zuerst, den bisherigen Eintrag wieder aufzunehmen — das gelingt, solange es dieselbe Maschine ist, und dann entsteht kein zweiter. Ist es eine andere Maschine, wird weiterhin neu angemeldet, und der ersetzte Eintrag wird dabei entfernt. *(Backend)*

- **Die Autostart-Sperre des VPN-Dienstes war in beide Richtungen unzuverlässig** — Beim Lösen der Verbindung soll der VPN-Dienst des Servers dauerhaft stillgelegt werden, damit er nach einem Neustart nicht von selbst in einer Instanz landet, die er nicht mehr verwaltet. Diese Sperre wurde jedoch weder zuverlässig gesetzt noch je wieder aufgehoben. Gesetzt wurde sie nur, wenn der Dienst zum Zeitpunkt des Lösens ohnehin schon gestoppt war — im Normalfall lief er, und sie blieb aus. War sie dagegen einmal gesetzt, blieb sie es für immer, auch nach einer erfolgreichen Neuanmeldung. Je nach Vorgeschichte konnte der Server nach einem Neustart also unerwartet zurückkehren oder unbemerkt aus dem VPN fallen. Beide Hälften sind jetzt verlässlich: Das Stilllegen erzwingt die Sperre, jede Neuanmeldung hebt sie auf. *(Backend)*

## 2026-08-22

- **Die Übernahme eines vorgefundenen VPN-Bestands ist vorläufig abgeschaltet** — Wer sich mit einer VPN-Instanz verbindet, auf der bereits Objekte stehen, konnte diese bisher übernehmen, statt sie zu bereinigen. Dabei aktualisiert der Server seinen eigenen Eintrag auf der Instanz nicht: der alte Eintrag bleibt stehen, und der übernommene Bestand ist damit nicht der, den der Server anschliessend verwaltet. Die Schaltfläche ist deshalb deaktiviert und nennt den Grund an Ort und Stelle. Der verlässliche Weg führt bis auf Weiteres über „Bereinigen" und einen Neuaufbau. *(Verwaltungsoberfläche)*

- **„Verbindung lösen": drei Randfälle abgesichert** — Fiel die Entscheidung genau in dem Moment, in dem im Hintergrund ein Abgleich lief, konnte dieser danach wieder Zuordnungen zur gerade aufgegebenen VPN-Instanz anlegen — mit demselben endlosen Abgleich als Folge. Das Lösen wartet jetzt auf einen laufenden Abgleich, und der Abgleich bricht ab, wenn die Verbindung ihm zwischenzeitlich entzogen wurde. Zweitens werden die DNS-Einträge der entfernten Freigaben in einem Durchgang abgeräumt statt einzeln — bei vielen Freigaben spart das ebenso viele Neustarts des DNS-Dienstes, und die Rückmeldung nennt jetzt genau die Namen, die wirklich verschwunden sind. Drittens überlebt der Vorgang einen Absturz zwischen Speichern und Nachziehen, ohne DNS-Einträge zurückzulassen, die niemand mehr findet. *(Backend)*

- **Bereinigen ohne vollständige Instanz-Auskunft entfernte zu viel** — Antwortete die VPN-Instanz beim Prüfen ihrer Fähigkeiten kurzzeitig mit einem Fehler, galt sie als „kennt keine Netzwerke" — und das Aufräumen nach dem Bereinigen hätte dann die Zuordnungen zu Netzwerken, Freigaben und Routern entfernt, obwohl diese Objekte dort weiterhin standen. Aufgeräumt wird jetzt nur, worüber die Instanz tatsächlich Auskunft gegeben hat. *(Backend)*

- **Nach dem Wiederverbinden mit der VPN-Instanz kam der Abgleich nie zur Ruhe** — Wer die Verbindung zur VPN-Verwaltung gelöst und danach wieder aufgenommen hatte, bekam einen Abgleich, der sich im Minutentakt an denselben Objekten festbiss und nie fertig wurde. Der Grund lag in der Datenbank: ThinForge merkt sich zu jedem angelegten Objekt, unter welcher Kennung es auf der Instanz steht — und diese Notizen blieben beim Lösen stehen. Nach dem Wiederverbinden zeigten sie auf Objekte, die es dort nicht mehr gab; ThinForge versuchte abwechselnd, sie neu anzulegen (der Name war belegt) und das Vorgefundene als fremd zu entfernen (es hing an einem Zugangsschlüssel). **„Verbindung lösen" räumt diese Notizen jetzt mit ab — zusammen mit den Freigaben dieser Installation und deren Regeln**, samt der zugehörigen DNS-Einträge. Wer seine Freigaben behalten will, sichert sie vorher über „Zustand → Sicherung". Auf der VPN-Instanz selbst wird weiterhin nichts angefasst, und die Zuordnung der Geräte zu ihren Zugängen bleibt ebenfalls bestehen. *(Backend, Verwaltungsoberfläche)*

- **Bereinigen ließ unsichtbare Reste in der Datenbank zurück** — Beim Leeren einer VPN-Instanz verlor ThinForge die Notiz zu jedem Objekt, das es einzeln gelöscht hatte. Manche Objekte werden aber gar nicht einzeln gelöscht: Freigaben verschwinden mit ihrem Netzwerk, und die von der VPN-Software selbst verwaltete Gruppe lässt sich nie entfernen. Deren Notizen blieben liegen und führten beim nächsten Verbinden zu demselben endlosen Abgleich. Nach dem Bereinigen wird jetzt auch das entfernt, wofür es keinen eigenen Löschvorgang gab. Bricht der Vorgang vorzeitig ab, bleiben die Notizen zu allem erhalten, was noch auf der Instanz steht. *(Backend)*

- **Die Erstverbindung zu einer frisch aufgesetzten VPN-Instanz kam nicht zustande** — Beim Anschluss an eine noch leere NetBird-Instanz brach der Vorgang mit einer Protokollmeldung ab. Die Ursache lag in der Antwort der Instanz selbst: Für eine leere Liste — etwa die Freigaben eines gerade erst angelegten Netzwerks — meldet sie „nichts" statt einer leeren Liste, und darauf war ThinForge nicht vorbereitet. Leere Listen werden jetzt als das gelesen, was sie sind. Eine wirklich fehlerhafte Antwort wird weiterhin gemeldet und nicht als „dort ist nichts" missverstanden — der Abgleich würde sonst den gesamten Bestand neu anlegen. *(VPN-Anbindung)*

- **Dienst-Anzeigen blieben auf dem Stand des Seitenaufrufs stehen** — Die Chips „aktiv/inaktiv" in der Netzwerk-, PXE- und VPN-Ansicht wurden einmal beim Öffnen geladen und danach nie wieder. Fiel ein Dienst während der Sitzung aus, zeigte die Seite unverändert grün — in der DHCP-Ansicht wirkte die dann leere Lease-Tabelle wie ein Netzwerkproblem, obwohl schlicht der Dienst gestorben war. Die Anzeigen aktualisieren sich jetzt von selbst; in der Netzwerk- und der PXE-Ansicht zieht der Aktualisieren-Knopf sie zusätzlich sofort mit. *(Verwaltungsoberfläche)*

- **Die Geräte-Detailseite aktualisierte sich nicht** — Status, „zuletzt gesehen" und die Live-Werte zu Arbeitsspeicher und Festplatte standen auf dem Stand des Seitenaufrufs, und es gab keinen Knopf dagegen. Ging ein Gerät währenddessen online, blieb es auf „offline", und Terminal wie Fernwartung blieben ausgegraut. Die Seite lädt jetzt selbstständig nach und hat einen Aktualisieren-Knopf. Angefangene Eingaben in den Gerätedaten bleiben dabei erhalten: solange etwas Ungespeichertes im Formular steht, wird es nicht überschrieben. *(Verwaltungsoberfläche)*

- **Neue Geräte-Anmeldungen tauchten nicht von selbst auf** — Meldete sich ein Thinclient an, während die Geräteliste offen war, erschien er dort nicht. Der zugehörige Aktualisieren-Knopf saß im Freigabe-Kasten selbst, und der wird bei leerer Liste gar nicht angezeigt — es gab also keinen Weg, ohne Neuladen der Seite nachzusehen. Wartende Anmeldungen erscheinen jetzt von selbst. *(Verwaltungsoberfläche)*

- **Das Backup-Fenster vergaß ein laufendes Backup** — Wer während eines Daten-Backups die Seite wechselte und zurückkam, fand ein scheinbar untätiges Fenster vor: kein Fortschritt, kein Abbrechen-Knopf, und der „Erstellen"-Knopf war wieder aktiv, brachte aber nur eine Fehlermeldung. Ein laufendes Backup wird jetzt beim Öffnen wieder aufgegriffen und mit Fortschritt angezeigt. *(Verwaltungsoberfläche)*

- **Kleinere Auffrischungen** — Die Client-Tabelle einer Gruppe zeigt Online-Status und VPN-Zahl jetzt fortlaufend statt nur zum Zeitpunkt des Gruppen-Klicks und hat einen eigenen Aktualisieren-Knopf. Die Warnliste zu doppelt vergebenen IP-Adressen frischt sich ebenfalls auf, statt bis zum Verlassen der Netzwerk-Ansicht stehenzubleiben. Und der Zähler am Ende der Ersteinrichtung reißt niemanden mehr auf die Anmeldeseite, der in den letzten Sekunden zurück in die Anwendung navigiert. *(Verwaltungsoberfläche)*

- **Das Dashboard aktualisierte nur die Hälfte von sich selbst** — Die Kennzahl-Kacheln oben wurden laufend nachgeladen, die Karten darunter nicht: Dienste-Status, offene Freigaben, laufende und fehlgeschlagene Aufgaben, Rollouts, Compliance und „zuletzt gesehen" standen auf dem Stand des Seitenaufrufs. Weil die Kacheln daneben sichtbar weitertickten, wirkte die ganze Seite aktuell — ein Admin sah dauerhaft „0 fehlgeschlagene Aufgaben" und übersah neu wartende Geräte. Es gab auch keinen Knopf, der das richtiggestellt hätte. Jetzt wird die vollständige Seite im selben Takt nachgeladen, und in der Kopfzeile steht ein Aktualisieren-Knopf. *(Verwaltungsoberfläche)*

- **„Aktualisieren" in den Berichten zeigte nie etwas Neues** — Der ausgewählte Zeitraum wurde beim Öffnen eines Berichts-Reiters festgenagelt, samt seinem Endzeitpunkt. Ein Klick auf „Aktualisieren" holte die Daten zwar wirklich neu, fragte aber weiterhin genau dieses alte Fenster ab: Aufgaben, Störungen, Rollouts oder SSH-Kommandos, die seit dem Öffnen dazugekommen waren, lagen dahinter und tauchten nicht auf — im SSH-Bericht kamen dadurch jedes Mal exakt dieselben Zahlen zurück. Der Zeitraum wird jetzt als „die letzten 7/30/90 Tage" gehalten und bei jedem Laden neu auf die aktuelle Uhrzeit bezogen. Betroffen waren acht Berichts-Reiter. *(Verwaltungsoberfläche)*

- **Die Deployment-Übersicht blieb genau im Moment des Abschlusses stehen** — Solange ein Deployment lief, aktualisierte sich die Übersicht von selbst; sobald das letzte fertig war, hörte sie schlagartig auf. Gerade dann arbeitet der Server aber noch: Die abschließende Aktion auf den Geräten — Neustart oder Herunterfahren — und die letzten Zeilen je Gerät folgen erst danach. Auf dem Bildschirm blieb deshalb der vorletzte Stand stehen, bis jemand von Hand aktualisierte. Die Übersicht lädt jetzt nach dem Abschluss noch einmal nach, samt der aufgeklappten Geräteliste, und meldet den Abschluss — bei fehlgeschlagenen Geräten als Warnung statt als Erfolg. *(Verwaltungsoberfläche)*

- **Nach dem Neustart eines einzelnen Geräts stand die Deployment-Übersicht still** — Wer in einem abgeschlossenen Deployment ein einzelnes fehlgeschlagenes Gerät erneut startete, bekam die Zeilen der Geräte aktualisiert — die Übersicht selbst nahm das Deployment aber nicht wieder als laufend an. Der Fortschritt des neu gestarteten Geräts blieb daraufhin stehen, und der Abschluss wurde nicht gemeldet. Die Übersicht greift das Deployment jetzt wieder auf. *(Verwaltungsoberfläche)*

## 2026-08-21

- **Verwaiste VPN-Zuordnungen lassen sich im Warndialog gleich mitentfernen** — Besteht keine Verbindung zur VPN-Verwaltung mehr, kehrte der Hinweis auf VPN-Geräte bei jedem Deployment wieder, weil die Zuordnungen niemand mehr auflöst. Der Dialog bietet dafür jetzt ein Kästchen an: angehakt, werden genau die aufgeführten Geräte als deaktiviert geführt, geben ihre Lizenzplätze frei und legen ihren VPN-Dienst beim nächsten Lebenszeichen still — der Hinweis erscheint für sie nicht mehr. Das Kästchen ist standardmäßig leer, erscheint nur bei gelöster Verbindung und nur für Administratoren. Die zugehörigen Zugänge auf der VPN-Instanz bleiben bestehen und sind dort separat zu entfernen. *(Backend, Verwaltungsoberfläche)*

- **Ein Deployment warnte vor VPN-Geräten, obwohl die VPN-Verbindung gar nicht mehr besteht** — Wer ein Deployment oder einen Rollout anlegte, bekam die betroffenen VPN-Geräte aufgelistet und musste bestätigen — auch auf Installationen, deren Verbindung zur VPN-Verwaltung gelöst ist. Die Meldung versprach dabei, ThinForge stelle anschliessend automatisch einen neuen Zugangsschlüssel aus; ohne bestehende Verbindung passiert genau das nicht. Die Vorschau prüft jetzt dasselbe wie das Deployment selbst. Besteht die Verbindung, bleibt alles wie bisher. Besteht sie nicht, sagt der Hinweis, was wirklich geschieht: Der Zugang dieser Geräte übersteht das Neuaufsetzen nicht und muss nach dem Wiederverbinden von Hand neu eingerichtet werden. *(Backend, Verwaltungsoberfläche)*

## 2026-08-20

- **VPN-Geräteliste zeigt an, wenn ein Tunnel über den Vermittlungsserver läuft** — Ob eine Verbindung direkt zustande kommt oder über den Vermittler der VPN-Instanz umgeleitet wird, meldet das Gerät seit jeher, sichtbar war es nirgends. Der Umweg ist spürbar langsamer und war unter anderem die Ursache dafür, dass Updates auf einzelnen Geräten stundenlang krochen. Betroffene Geräte tragen jetzt einen Hinweis in der Spalte „Verbindung", samt Erklärung beim Überfahren. Direkte Verbindungen bleiben unmarkiert — der Hinweis soll auffallen, nicht in jeder Zeile stehen. *(Verwaltungsoberfläche)*

- **Platzhalter „VPN-Statistik" aus der Geräteliste entfernt** — Unter der VPN-Geräteliste stand dauerhaft ein leerer Kasten mit dem Hinweis, eine Auswertung folge später. Die technische Grundlage dafür gibt es seit dem Umbau der VPN-Anbindung nicht mehr, das Versprechen stand also ohne Aussicht im Bild. Der Kasten ist weg; an den Angaben je Gerät ändert sich nichts. *(Verwaltungsoberfläche)*

- **Ein Gerät, dem der VPN-Zugang entzogen wurde, versuchte danach dauerhaft weiter, sich zu verbinden** — Der Entzug wirkt auf der VPN-Instanz: Der Zugang des Geräts wird dort gelöscht, und damit kommt es nicht mehr hinein. Zum Gerät selbst ging bisher aber keine Nachricht. Es behielt seine wertlos gewordenen Zugangsdaten, hielt den VPN-Dienst am Laufen und klopfte im Minutentakt bei einer Gegenstelle an, die es nicht mehr kennt — auch über einen Neustart hinweg. In der Geräteliste stand es zudem dauerhaft auf „pausiert" statt auf „nicht aktiviert", weil es sich selbst weiterhin für angemeldet hielt. Der Server teilt jedem Gerät jetzt in jedem Lebenszeichen mit, ob sein VPN-Zugang bestehen soll. Fällt er weg, legt das Gerät den Dienst still und räumt die verbliebenen Zugangsdaten ab; die Anzeige stimmt damit wieder. Geräte, denen der Zugang bereits vor dieser Aktualisierung entzogen wurde, sind eingeschlossen: sie werden beim Einspielen einmalig nacherfasst und legen den Dienst beim nächsten Lebenszeichen still. Am Entzug selbst ändert sich nichts — er wirkt weiterhin sofort auf der VPN-Instanz, unabhängig davon, ob das Gerät gerade erreichbar ist. *(Backend, Agent)*

## 2026-08-17

- **Updates krochen über die VPN-Verbindung mit 1 Mbit, obwohl die Leitung viel mehr hergab** — Die Bandbreiten-Regelung wertet die Laufzeitschwankung der Verbindung aus, um einer parallelen Sitzung nicht im Weg zu stehen. Auf Verbindungen über den VPN-Vermittler schwankt die Laufzeit von Haus aus stark genug, dass die Regelung diese Schwankung für selbstverursachten Stau hielt: Sie drosselte im Zwei-Sekunden-Takt und kam nie wieder hoch — ein Update von 450 MB brauchte damit Stunden statt Minuten. Die Regelung rechnet die Schwankung jetzt heraus, drosselt nicht mehr unterhalb der eigenen Erlaubnis und bricht aus einer Dauerdrosselung selbsttätig wieder aus. Das Zurückweichen vor anderem Verkehr bleibt erhalten. *(Agent v2.18.0)*

- **VPN-Clients ausserhalb des Hauses konnten den Servernamen nicht auflösen** — Auf Installationen, die vor dem 2026-08-04 eingerichtet und seitdem nicht mehr in den Netzwerk-Einstellungen gespeichert wurden, fehlte der DNS-Dienst eine Zeile: Er hörte nur im Gerätenetz zu und verwarf jede Namensanfrage, die über den VPN-Tunnel hereinkam — ohne Meldung. Betroffene Geräte fragten daraufhin den DNS-Server ihres eigenen Standorts, der den Servernamen nicht kennt, und meldeten sich nicht mehr, obwohl ihr Tunnel stand. Das Backend prüft die erzeugte Konfiguration jetzt bei jedem Start gegen den aktuellen Stand, erneuert sie bei Abweichung und startet den DNS-Dienst dabei neu. *(Backend)*

- **Eine frisch eingerichtete VPN-Verbindung blieb auf „nicht eingerichtet" stehen** — Nach „Server anmelden" nahm der Abgleich die eben abgeschlossene Einrichtung sofort wieder zurück: Der Schutz, der nach einem Datenbankverlust den Bestand rettet, hielt schon die Gruppe und den Servereintrag für eine stehende Konfiguration, die es zu bewahren gilt — beides legt die Anmeldung aber selbst an. Der Assistent meldete danach unverändert „Die Erstverbindung ist noch nicht abgeschlossen", und die VPN-Objekte entstanden nie. Der Schutz greift jetzt erst bei wirklich angewandten Objekten; nach einem Datenbankverlust bleibt er unverändert wirksam. *(Backend)*

- **„Verbindung lösen" beendet jetzt auch den eigenen VPN-Zugang des Servers** — Bisher entfernte der Befehl nur Adresse, Zugangs-Token und Prüfergebnis. Der Server selbst blieb mit laufendem Tunnel im VPN und meldete dort weiterhin die Wege in sein Gerätenetz — sichtbar nirgends, und über die Oberfläche auch nicht mehr zu beenden. Der eigene Zugang wird jetzt zuerst beendet, und zwar dauerhaft: Auch nach einem Neustart verbindet sich der Dienst nicht mehr von selbst. Auf der VPN-Instanz bleibt weiterhin alles unangetastet. Gelingt das Beenden nicht, ist die Verbindung trotzdem gelöst — die Oberfläche sagt dann, dass der Tunnel von Hand zu prüfen ist. *(Backend, Verwaltungsoberfläche)*

- **Der Einrichtungsassistent überspringt die Server-Anmeldung nicht mehr, wenn sie fehlt** — Übernimmt eine **zweite** ThinForge-Installation den Bestand einer ersten, gehört der darin verzeichnete Servereintrag noch dem alten Server. Der Assistent sprang trotzdem direkt zur Zusammenfassung: Die Einrichtung galt als fertig, während auf der neuen Maschine kein einziger Tunnel lief und die Wege ins Gerätenetz weiter auf den alten Server zeigten. Er führt jetzt zur Server-Anmeldung, samt Begründung. *(Backend, Verwaltungsoberfläche)*

- **Eine Migration konnte einen Port öffnen, den niemand freigegeben hatte** — Trugen auf einem Bestandssystem zwei Host-Module denselben Namen und unterschieden sich nur in der Groß-/Kleinschreibung, kam beim Update wie angekündigt das ältere als Freigabe durch. Die **Zugriffsregeln** des übergangenen Moduls landeten dabei aber an der Freigabe des durchgekommenen — auch dann, wenn dieses Modul abgeschaltet war und auf einen ganz anderen Rechner zeigte. Damit stand ein Port auf einem Rechner offen, für den ihn nie jemand freigegeben hatte. Die Regeln des übergangenen Moduls fallen jetzt mit ihm weg; das Protokoll nennt es weiterhin namentlich. *(Backend)*

- **Geht die Datenbank verloren, bleibt die VPN-Einrichtung jetzt stehen** — Verlor ThinForge seinen VPN-Teil der Datenbank, während die VPN-Instanz weiterlief, hielt der Abgleich den kompletten Bestand für fremd und räumte ihn binnen einer Minute ab — Freigaben, Netzwerkobjekte und Router. Genau dieser Bestand ist aber die Grundlage für „Übernehmen" im Einrichtungsassistenten, den Weg zurück nach einem Datenverlust; er war weg, bevor jemand den Assistenten öffnen konnte. Der Abgleich erkennt die Lage jetzt und hält an: **es wird nichts gelöscht**, der Einrichtungszustand geht auf „nicht eingerichtet" zurück, und der Assistent bietet die Übernahme an. Die Geräte kommen dabei wie bisher ohne Neuanmeldung zurück. *(Backend)*

- **Der Abgleich kam auf migrierten Installationen nie zur Ruhe** — Übernommene Host-Module trugen ihre Zieladresse in der Schreibweise der alten Verwaltung; die VPN-Verwaltung gibt sie in einer anderen zurück. Der Abgleich sah deshalb auf ewig eine Abweichung und schrieb dieselbe Freigabe jede Minute neu — erfolgreich, endlos, für jede übernommene Freigabe. Der Zustand „abgeglichen" wurde nie erreicht. Adressen werden jetzt beim Übernehmen und beim Einspielen einer Sicherung in dieselbe Form gebracht, die auch der Freigaben-Assistent verlangt. *(Backend)*

- **Das Einspielen einer Sicherung räumt DNS-Namen mit ab** — Entfernt eine Sicherung eine Freigabe, die es hier noch gab, verschwand bisher alles außer ihrem DNS-Namen: das Gerätenetz löste ihn weiter auf ein Ziel auf, das niemand mehr freigegeben hatte. Der Name fällt jetzt im selben Durchgang, in dem die übrigen nachgezogen werden — weiterhin einmal für alle Einträge. Namen, die ThinForge selbst gehören, bleiben dabei unangetastet. *(Backend)*

## 2026-08-15

- **Die VPN-Anbindung ist neu gebaut — ein Soll-Zustand statt Einzelbefehlen** — ThinForge beschreibt jetzt, wie die VPN-Verwaltung auszusehen hat, und gleicht diesen Stand im Minutentakt ab, statt jede Änderung einzeln hinzuschicken. Abweichungen werden geheilt, Objekte ohne Entsprechung entfernt, und jede Aktion landet mit Ausgang und Fehlergrund in einem Protokoll, das der neue Tab „Zustand" zeigt — zusammen mit „Prüfen" und „Jetzt anwenden". Daraus folgt die wichtigste neue Betriebsregel: **im VPN-Dashboard wird nichts mehr von Hand gepflegt**, dort angelegte Objekte verschwinden beim nächsten Abgleich. *(Backend, Verwaltungsoberfläche)*

- **Freigaben lösen die Host-Module ab — mit Assistent und Vorschau** — Ein Ziel aus dem Client-Netz über das VPN erreichbar zu machen, ist jetzt ein Assistent in vier Schritten: Ziel (mit optionalem DNS-Namen), Zugriffsregeln mit Vorlagen für RDP, VNC, HTTPS und SSH, Vorschau genau der Objekte, die entstehen, und Anwenden mit anschließender Prüfung. Ports sind dabei Einzelwerte — einen Bereich wie 3000-4000 kennt die Kette nicht. Vorhandene Host-Module werden beim ersten Start nach dem Update automatisch als Freigaben übernommen; tragen zwei Alt-Module denselben Namen und unterscheiden sich nur in der Groß-/Kleinschreibung, kommt das ältere durch — das übergangene steht namentlich im Protokoll und ist neu anzulegen. *(Backend, Verwaltungsoberfläche)*

- **Die Erstverbindung fragt, was mit einem vorhandenen Bestand geschehen soll** — Der Einrichtungsassistent liest die VPN-Instanz und zeigt, was er vorfindet: leer, die eigene frühere Einrichtung, die einer anderen Installation, eine Alt-Konfiguration oder etwas gänzlich Fremdes. Erkennt er die eigene Einrichtung wieder, lässt sie sich **übernehmen** — die Zuordnung wird aus dem Bestand rekonstruiert, und **kein Gerät muss sich neu anmelden**, bestehende Tunnel bleiben stehen. Das ist der Weg zurück, wenn die Datenbank verloren geht. Eine Grenze hat er: die DNS-Namen von Freigaben stehen nirgends auf der Instanz. Steht die Freigabe hier noch, bleibt ihr Name deshalb einfach stehen — neu einzutragen ist er nur, wenn auch die Datenbank verloren war (oder man spielt eine Exportdatei ein, die ihn mitführt). Freigaben, die es hier schon gibt — auf einem Bestandssystem übernimmt der erste Start nach dem Update die bisherigen Host-Module —, werden dabei zusammengeführt statt verdoppelt. Andernfalls zeigt der Assistent ein Inventar und löscht erst nach ausdrücklicher Bestätigung. Von selbst geschieht nichts davon. *(Backend, Verwaltungsoberfläche)*

- **Ein im VPN-Dashboard von Hand gelöschtes Gerät heilt nicht von selbst** — Wird der Zugang eines Geräts direkt in der VPN-Verwaltung gelöscht, zeigt ThinForge es als getrennt, löst die Zuordnung aber bewusst nicht auf: nach einer Neuanbindung ist genau diese Zuordnung der einzige Hinweis auf den noch abzuräumenden Alt-Eintrag. Der Weg zurück ist **„VPN deaktivieren" und danach wieder „VPN aktivieren"** — das Gerät bekommt dabei einen frischen Zugangsschlüssel. Ein Reimage oder ein Deploy deaktiviert die betroffenen Geräte ohnehin von sich aus. *(Backend)*

- **„Verbindung lösen" räumt nur noch auf dieser Seite auf** — Der Befehl entfernt Adresse, Zugangs-Token und Prüfergebnis hier; die VPN-Instanz selbst bleibt unangetastet, ihre Netzwerke, Regeln und Gerätezugänge arbeiten weiter. Das ist Absicht: eine Instanz zu leeren ist eine andere, weitreichendere Entscheidung. Getroffen wird sie im Erstverbindungs-Assistenten, der den Bestand zuerst als Inventar zeigt und ihn erst nach Bestätigung löscht. *(Backend, Verwaltungsoberfläche)*

- **Ein zweiter Klick auf „Server registrieren" fasst nichts mehr an** — Auf einer fertig eingerichteten Installation gibt der Schritt jetzt den gespeicherten Stand zurück, statt den Server ein zweites Mal anzumelden — kein Trennen des lokalen Tunnels, kein zweiter Zugangsschlüssel und auch kein Neuladen des Webservers. Ein erneutes Anmelden hätte einen zweiten Servereintrag hinterlassen: gültiger Zugang, keine Zuordnung, und die Routen zeigten auf die falsche Identität. Wer die VPN-Objekte neu anwenden möchte, nimmt dafür „Jetzt anwenden" im Tab „Zustand". Fehlt der Servereintrag dagegen auf der Instanz, läuft die Anmeldung wieder vollständig durch. *(Backend)*

- **Die VPN-Einrichtung lässt sich sichern und wieder einspielen** — Der Tab „Zustand" exportiert Freigaben, Regeln und Gerätezuordnungen als Datei und liest sie ebenso wieder ein; das Zugangs-Token bleibt dabei ausdrücklich draußen, es ist an diese Installation gesiegelt. Freigaben mit DNS-Namen werden nach dem Einspielen in der Namensauflösung nachgezogen — einmal für alle Einträge, und genauso nach einer Übernahme im Einrichtungsassistenten. Gerätezuordnungen ohne passendes Gerät werden übersprungen und stehen **nur im Protokoll**. Empfohlen wird zusätzlich eine regelmäßige Sicherung der VPN-Instanz selbst (`store.db`): sie liegt außerhalb der Reichweite von ThinForge und ist die Vorbedingung dafür, dass Geräte außerhalb des Hauses einen Instanzverlust ohne Vor-Ort-Termin überstehen. *(Backend, Verwaltungsoberfläche)*

- **Schreibende VPN-Aktionen sind Administrator-Sache** — Aktivieren, Deaktivieren, das Aktivieren einer ganzen Gruppe, das Neuanbinden der Flotte, Freigaben anlegen und ändern, Prüfen und Anwenden, Export und Import sowie Verbindungstest und Bestandsprüfung der Instanz setzen jetzt durchgängig Administratorrechte voraus. Das gilt auch für die Erstverbindung: auf einer noch nicht eingerichteten Installation sehen Operatoren jetzt einen Hinweis statt des Assistenten, der ihnen bei jedem Schritt eine Fehlermeldung geliefert hätte. Operatoren behalten die vollständige Leseansicht; die übrigen Bedienelemente sind bei ihnen ausgeblendet oder gesperrt, statt beim Klick zu scheitern. *(Backend, Verwaltungsoberfläche)*

- **Geräte lassen sich neu anbinden, ohne je zwei Tunnel zu haben** — Zieht die Flotte auf eine andere VPN-Instanz um, tauscht der Agent seine Konfiguration in einem Zug: sichern, alten Tunnel abbauen, neu anmelden, prüfen — und zwar nicht nur, ob der Tunnel steht, sondern ob er den Server auch trägt. Scheitert ein Schritt, stellt das Gerät die alte Konfiguration selbst wieder her und meldet, woran es lag; ohne tragfähiges Netz wird der laufende Tunnel gar nicht erst abgerissen. Geräte außerhalb des Hauses bleiben so erreichbar. Neu ab Agent v2.17.0. *(Agent, Backend)*

- **Nach dem Update ist der VPN-Einrichtungsassistent einmal zu durchlaufen** — Die neue VPN-Anbindung startet auf jeder Installation als „noch nicht eingerichtet“, auch dort, wo das VPN vorher lief. Bis der Assistent einmal mit dem Zugriffstoken der VPN-Verwaltung durchlaufen ist — bei vorhandenem Bestand über **Übernehmen** —, **ruht der Abgleich**: die Statusleiste meldet „Einrichtung nicht abgeschlossen“, Geräte zeigen ihren Verbindungszustand als unbekannt, und es wird nichts angelegt und nichts entfernt. Bestehende Tunnel laufen dabei weiter; ThinForge weiß nur nichts über sie. Bisherige Host-Module sind zu diesem Zeitpunkt bereits als Freigaben übernommen. *(Backend, Verwaltungsoberfläche)*

## 2026-08-12

- **Ein langer Wert legt den Statusbericht eines Geräts nicht mehr lahm** — Zwei Felder, deren Länge nicht der Server bestimmt, hatten eine feste Obergrenze: die Agent-Version und die Liste der verfügbaren Wiederherstellungspunkte. Wurde sie überschritten, scheiterte nicht nur das Feld, sondern der komplette Statusbericht — das Gerät erschien offline, obwohl es lief. Beide Felder sind jetzt unbegrenzt. *(Backend)*

- **System-Gruppen lassen sich nicht mehr umbenennen** — Die drei vom System angelegten VPN-Gruppen waren gegen Löschen geschützt, gegen Umbenennen nicht. Ein Umbenennen hätte die Flotte still gespalten: die Gruppe wird über ihren Namen aufgelöst, also leer neu angelegt, während die bisherigen Mitglieder ohne Meldung ihren Zugriff verlieren. Beide Wege sind jetzt gleich geschützt. Außerdem: eine Bestandsprüfung, die nur wegen einer gestörten Abfrage leer aussah, gilt nicht mehr als leere Instanz. *(Backend, Verwaltungsoberfläche)*

- **Der Zustand „Konfiguration liegt am Client" kommt jetzt wirklich an** — Ist die VPN-Konfiguration auf einem Gerät hinterlegt, das Enrollment steht aber noch aus, meldet der Agent das seit v2.16.9. Die Datenbank wies diesen Zustand ab — und weil der VPN-Bericht eines Signals aus einem Stück besteht, ging dabei der ganze Bericht verloren: auch die Fehlerursache, also genau die Angabe, die erklärt, warum das Gerät nicht ins VPN kommt. Alle zehn Sekunden, dauerhaft. Der Zustand ist jetzt zugelassen, der Bericht kommt vollständig an. *(Backend)*

- **Abgebrochene VPN-Vorgänge lassen keine stillen Reste mehr zurück** — Schlug die Anmeldung des Servers beim VPN-Dienst fehl, blieb der bereits ausgestellte, unverbrauchte Zugangsschlüssel dort stehen: er gewährte weiter Zutritt, war aber nirgends verzeichnet und wurde deshalb auch beim Aufräumen nie gefunden. Er wird jetzt zurückgenommen. Ebenso meldet das Anlegen einer VPN-Gruppe jetzt einen Fehler, wenn der Eigentumsvermerk nicht geschrieben werden konnte — ohne ihn räumt der spätere Rückbau die Gruppe nicht mit ab. *(Backend)*

- **Ein Server-Wechsel ersetzt den Vorgänger jetzt wirklich** — Zieht die Verwaltung auf einen neuen Server um, meldet sich dieser beim VPN-Dienst unter demselben Namen an — es ist dieselbe Rolle. Bisher blieb der alte Eintrag daneben stehen, und Routen und Regeln zeigten weiter auf ihn: das Rechnernetz war über das VPN nicht mehr erreichbar, ohne dass irgendetwas einen Fehler gemeldet hätte. Der abgelöste Eintrag wird jetzt entfernt und alles Verbliebene auf den neuen Server umgehängt. Lässt er sich nicht entfernen, sagt das Protokoll, was am VPN-Dienst noch zu tun ist. *(Backend)*

- **„Unbekannt" ist keine Trennung mehr** — Solange der Abgleich mit dem VPN-Dienst klemmt, weiß der Server über einzelne Geräte schlicht nichts. Die Liste zeigte das trotzdem als „getrennt" — bei stehendem Abgleich also die ganze Flotte. Sie unterscheidet jetzt drei Zustände: verbunden, getrennt und unbekannt. *(Verwaltungsoberfläche, Backend)*

- **Ein VPN-Zugang gehört jetzt garantiert zu genau einem Gerät** — Wurde ein Gerätename umbenannt und später wiederverwendet, konnten zwei Geräte auf dieselbe Gegenstelle im VPN zeigen. Das Abschalten des einen entfernte dann den Zugang des anderen. Die Zuordnung wird jetzt in zwei getrennten Schritten vergeben und in der Datenbank als eindeutig erzwungen; bestehende Doppelzuordnungen werden beim Aktualisieren aufgelöst, die betroffenen Geräte melden sich von selbst neu. *(Backend)*

- **„Aktualisieren" meldet keinen Erfolg mehr, wenn nichts abgeglichen wurde** — Gehörte die Geräteliste des VPN-Dienstes erkennbar nicht zu diesem Server, brach der Abgleich stillschweigend ab und meldete trotzdem Erfolg mit null abgeglichenen Geräten. Jetzt nennt er den Grund. Beim Lösen der Verbindung ohne Abräumen werden außerdem die gespiegelten VPN-Adressen entfernt — sie waren von diesem Server aus nicht mehr erreichbar, wurden aber weiter für automatisierte Zugriffe verwendet. *(Backend, Verwaltungsoberfläche)*

- **Der Rückbau bleibt nicht mehr an Kleinigkeiten hängen** — Beim Entfernen des VPN-Bestands konnten drei Dinge dauerhaft blockieren: eine eigene Gruppe, die noch in den DNS-Einstellungen des Dienstes hinterlegt war, alte Routen aus einem früheren Modell, und eine Verknüpfung, die sich nur dort selbst lösen lässt. Die ersten beiden räumt der Server jetzt selbst ab; für die dritte nennt die Meldung, was am VPN-Dienst zu tun ist, statt auf einen Wiederholversuch zu verweisen, der nie gelingen kann. Solche Alt-Routen zählen außerdem beim Verbinden als vorhandener Bestand. *(Backend)*

- **Ein abgebrochener Werksreset spukt nicht mehr nach** — Die Liste der Geräte, die ein Werksreset wiederherstellen soll, überlebte bisher jeden Rückbau. Wurde die Verbindung stattdessen gelöst und später eine neue eingerichtet, holte der nächste Werksreset die komplette alte Geräteliste zurück — samt Lizenzabsage über Geräte, die niemand aktiviert hatte. Die Liste wird jetzt mit dem Bestand aufgegeben; beim Lösen ohne Abräumen bleibt sie bewusst erhalten. *(Backend)*

- **Ein Gerät kommt nach dem Neuaufsetzen wieder ins VPN** — Wurde der VPN-Zugang eines Geräts auf dem Dienst gelöscht, etwa beim Neuaufsetzen oder nach einem fehlgeschlagenen Aufräumen, behielt der Server die alte Zuordnung. Der Einrichtungsauftrag setzt aber genau voraus, dass keine besteht — das Gerät kam damit still nie wieder ins VPN, ohne Fehlermeldung. Zuordnungen auf gelöschte Gegenstellen werden jetzt aufgelöst, sofern das Gerät sie nie selbst bestätigt hat. *(Backend)*

- **Eigene Ergänzungen auf dem VPN-Dienst bleiben erhalten** — Wurden dort Beschreibungen, zusätzliche Zugangsbedingungen, weitere Teilregeln, Gruppen oder Nameserver ergänzt, entfernte der nächste Abgleich sie wortlos: er kannte sie nicht und schrieb die Objekte vollständig neu. Beschreibungen und Zugangsbedingungen werden jetzt mitgeführt; Objekte, deren Erweiterung sich hier gar nicht abbilden lässt, bleiben unangetastet und werden im Konfigurations-Reiter benannt. *(Backend)*

- **Der Wechsel auf eine andere VPN-Instanz wird erkannt und abgefragt** — Wurden Zugangsdaten eingetragen, die auf eine andere Instanz zeigen als die, auf der dieser Server registriert ist, richtete er sich dort kommentarlos neu ein. Die Geräte der bisherigen Instanz behielten ihren Zugang, während der Server die einzige Zuordnung zu ihnen verlor; wer dabei „Löschen" wählte, bekam sogar Erfolg gemeldet, obwohl nichts entfernt wurde. Der Server weist das jetzt ab und erklärt es. Wird das Aufgeben der bisherigen Instanz ausdrücklich bestätigt, sagt der Dialog vorher, was dort zurückbleibt. *(Backend, Verwaltungsoberfläche)*

- **VPN-Verbindung lösen — wahlweise mit oder ohne Abräumen auf dem VPN-Server** — Das Lösen der Verbindung entfernte bisher immer alle vom Server angelegten Objekte auf dem VPN-Dienst. Für einen Server-Neuaufbau oder den Umzug auf ein anderes ThinForge ist das der falsche Weg: die Geräte sollen im VPN bleiben und weiterarbeiten. Der Dialog stellt die beiden Wege jetzt vor der Bestätigung zur Wahl und nennt zu jedem die Folge; voreingestellt bleibt das bisherige Verhalten. *(Verwaltungsoberfläche, Backend)*

- **Beim Verbinden wird vor vorhandenen Daten gewarnt** — Liegen auf dem VPN-Dienst bereits Daten, richtete ThinForge sich bisher ungefragt darin ein. Vor dem Speichern der Zugangsdaten erscheint jetzt eine Übersicht des vorhandenen Bestands samt der Geräte, die hier unter demselben Namen bekannt sind, und die Wahl zwischen Übernehmen und Löschen. Bis zur Entscheidung wird nichts gespeichert und nichts verändert; ohne Entscheidung lehnt der Server das Verbinden ab. *(Verwaltungsoberfläche, Backend)*

- **Übernommene Geräte werden ihren Einträgen wieder zugeordnet** — Der Zugangsschlüssel eines Geräts ist nach der Einrichtung verbraucht, die VPN-Verbindung besteht danach unabhängig vom Server weiter. Nach einem Neuaufbau oder Umzug ließen sich diese Geräte deshalb nicht mehr zuordnen, obwohl sie im VPN arbeiten. Sie werden jetzt über ihren Gerätenamen automatisch wieder verbunden — nur bei eindeutigem Namen und nur, wenn die Verbindung tatsächlich besteht. *(Backend)*

- **Der Konfigurations-Reiter zeigt fremde Geräte mit ihrem Verbindungszustand** — Geräte auf dem VPN-Dienst, die dieses ThinForge nicht verwaltet, standen nur als Namensliste da. Jetzt ist je Eintrag sichtbar, ob die Verbindung besteht und ob ein Gerät dieses Namens hier bekannt ist. Das unterscheidet die eigene Flotte nach einem Umzug von Geräten einer fremden Installation. *(Verwaltungsoberfläche, Backend)*

- **VPN-Geräte verschwinden nicht mehr aus der Übersicht** — Wurde der VPN-Haken einer Gruppe entfernt oder ein Gerät aus seiner Gruppe gelöst, verlor es weder den VPN-Zugang noch seinen Lizenzplatz — es verschwand nur aus der Übersicht, und damit die einzige Möglichkeit, es abzuschalten. Solche Geräte werden jetzt weiterhin angezeigt, gekennzeichnet und lassen sich dort deaktivieren. Der Hinweis beim Löschen einer VPN-Gruppe versprach zudem das Gegenteil dessen, was tatsächlich passiert, und wurde richtiggestellt. *(Backend, Verwaltungsoberfläche)*

- **Der Abgleich fasst nur noch an, was ihm gehört** — Eine über die Oberfläche angelegte VPN-Gruppe wurde beim nächsten Abgleich gelöscht oder hielt die Anzeige dauerhaft auf „nicht synchron"; gleichnamige Gruppen konnten neu entstehen und Regeln an die falsche binden; ein auf dem VPN-Dienst abgeschaltetes Objekt wurde entweder stillschweigend wieder eingeschaltet oder als „synchron" gemeldet, obwohl es wirkungslos war. Gruppen werden jetzt nicht mehr gelöscht, ein zweiter gleicher Name wird abgelehnt, und ein abgeschaltetes Objekt wird gemeldet statt umgeschaltet — auch dann, wenn nur die Regel darin abgeschaltet ist und das Objekt selbst noch aktiv aussieht. *(Backend, Verwaltungsoberfläche)*

- **Die Verbindungsanzeige sagt, was sie tatsächlich weiß** — Eine leere oder fehlerhafte Antwort des VPN-Dienstes wurde bisher als Tatsache übernommen und konnte die ganze Flotte als getrennt anzeigen; ein abgelaufener Zugriffsschlüssel blieb unbemerkt, weil der Status trotzdem grün blieb. Fehlende Daten gelten jetzt als „nichts bekannt" statt als „getrennt", der Status kennt einen eigenen Fehlerzustand mit Grund, und ein unlesbarer Zugriffsschlüssel nennt beide Auswege statt einer technischen Meldung. Zusätzlich wird gemeldet, wenn eine fremde Regel die Zugriffsbeschränkung des VPN aufhebt. *(Backend, Verwaltungsoberfläche)*

- **VPN-Einrichtung funktioniert in Netzen ohne Internetzugang** — Die Einrichtung eines Geräts prüfte vorab die Erreichbarkeit einer öffentlichen Adresse und brach in abgeschotteten Netzen ab, obwohl der VPN-Dienst im eigenen Netz erreichbar war. Maßgeblich ist jetzt allein die Erreichbarkeit des VPN-Dienstes. Außerdem wurde der Einrichtungsauftrag bei jedem Melden wiederholt und riss dabei die gerade aufgebaute Verbindung wieder ab — er wird jetzt gedrosselt und ein bereits verbrauchter Schlüssel erkannt. *(Geräte-Agent, Backend)*

- **Werksreset und Übernehmen führen zu Ende oder sagen, warum nicht** — Beim Werksreset blockierte ein nebensächlicher Konfigurationsfehler das Abschließen der Merkliste, gelöschte Geräte blieben als Karteileichen darin stehen, und ein bewusst abgeschaltetes Gerät wurde ungefragt wieder eingerichtet. Beim Übernehmen der Server-Identität war ein Wiederholversuch nach einem Abbruch nicht mehr möglich, und ein fehlgeschlagenes Löschen wurde als Erfolg gemeldet. Beides ist behoben; zusätzlich ist „noch nicht eingerichtet" jetzt ein eigener Zustand mit Knopf statt einer grünen Erfolgsmeldung. Übernommene Geräte zählt der Werksreset jetzt mit — bisher fehlten sie in der Merkliste und verloren beim Zurücksetzen lautlos den Zugang. *(Backend, Verwaltungsoberfläche)*

- **Sicherungen behalten den Aktivierungsstand der Geräte** — Beim Erstellen einer Systemsicherung wurde das Schlüsselmaterial der VPN-Geräte entfernt — versehentlich zusammen mit dem Vermerk, dass ein Gerät überhaupt aktiviert war. Nach dem Einspielen galten aktivierte Geräte als nie eingerichtet. Der Vermerk bleibt jetzt erhalten, nur das Schlüsselmaterial wird entfernt. *(Backend)*

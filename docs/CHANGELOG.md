...
- **Dienste-Status: Verwaltungsoberfläche wird nicht mehr fälschlich als „unhealthy" angezeigt** — Seit der Umstellung auf ein gehärtetes, minimales Laufzeit-Image meldete die Container-Zustandsprüfung die Verwaltungsoberfläche dauerhaft als „unhealthy", obwohl sie einwandfrei lief: Die alte Prüfung setzte eine Shell und Werkzeuge voraus, die es in diesem Image bewusst nicht gibt. Die Zustandsprüfung läuft jetzt über eine winzige eigenständige Prüfkomponente, die mit ins Image eingebaut wird — der Status wird wieder korrekt ermittelt, ohne die Härtung des Images aufzuweichen. Wirkt nach Aktualisierung der Verwaltungsoberfläche (Image) und der `docker-compose.yml` des Deploy-Repos.

- **Deployments beziehen nur noch getestete, versionierte Container-Images** — Die Bereitstellung wird auf ein Zwei-Kanal-Modell umgestellt: Entwicklungsstände werden in der Registry als `beta` geführt, produktive Deployments beziehen ab sofort fest verankerte Versions-Stände (`vJAHR.MONAT.TAG`), die erst nach bestandenem Test freigegeben werden. Das `docker-compose.yml` des Deploy-Repos verweist künftig auf diese Versions-Kennungen statt auf `latest`; `latest` selbst zeigt ab der ersten Freigabe ebenfalls auf den neuesten getesteten Stand — ein Deployment kann damit nicht mehr versehentlich einen ungetesteten Zwischenstand ziehen. Für bestehende Installationen ändert sich am Ablauf nichts (`git pull` und `./deploy.sh` wie gehabt); ein Zurückgehen auf einen früheren Stand ist über die Historie des Deploy-Repos jederzeit möglich, da alle Versions-Stände in der Registry erhalten bleiben.

...
- **Agent: updates with a new kernel boot reliably (comes with the next agent release)** — When applying an update, the agent carried over the boot configuration (GRUB) from the old system. If the update brought a new kernel, the device kept booting the old one or, if the old one was missing from the update, did not boot at all; a rollback across a kernel update always failed. The boot configuration is now generated from the new system itself after the switch and checked; if it does not fit, the device stays on its current version and the agent logs the failure. After a rollback, the snapshot menu offers the version that was left instead of a copy of the running one. *(Agent)*
- **Vulnerability scan: acceptance per image** — An entry of the acceptance list can be limited to specific services with an `- **Images:**` line; without that line it applies to all images as before. If another image reports the same ID, it appears there as a new finding instead of being covered silently. For a limited entry, the panel shows which images the acceptance applies to; the report names the images where it took effect and names that match no service. All 57 entries are limited to their images. Findings without a CVSS vector that an entry accepts for their image no longer count in the `needs_review` notice; the image list shows them as accepted. *(Backend, management interface, security)*
- **Scanners syft 1.52.0 and grype 0.119.0, NetBird 0.79.0** — The new scanner versions fix three high-severity findings in their own binaries in backend and worker. The current NetBird image ships the fixed OpenSSL version; the server pulls it with the next update. The acceptance list has been reconciled with the scan of 24 September: new entries with a justification, fixed ones removed. The scan then reports no new high-severity finding (eight before). Wrong justifications are corrected: the py3-redis findings in the cloning VM image are false positives (Redis server vulnerabilities matched to the Python client), and most OVMF findings arise only from the version scheme. *(Backend, worker, VPN, security)*

## 2026-09-24 (round 4)

- **The 285 low-severity findings of the 2 September full review are done** — Split by category into 23 work packages, each finding re-checked against the current code: 226 are fixed, 25 decisions implemented, one is deferred. The entries below summarise what becomes visible in operation; internal clean-ups without visible effect come on top. *(Backend, worker, Agent, management interface, server services)*
- **Remote Desktop: limits for idle time and sessions, screen connection only for the broker** — Settings → Remote Desktop now accepts 30 seconds to 24 hours of idle time and 1 to 50 sessions per device; 0 no longer means "unlimited", a stored 0 is read as 24 hours or 5 sessions. If the settings cannot be loaded, saving stays disabled instead of taking over the defaults. On the server, a session's screen connection is reachable only by the Guacamole service, no longer by other containers or processes, and a malformed response from that service now ends only the session instead of the whole backend. *(Backend, management interface, security)*
- **Terminal and remote control no longer take the login token from the URL** — The token now comes only from the login cookie or the Authorization header; the interface already works this way. **Switch your own scripts that open the terminal or remote control with `?token=` before the update.** The progress of a delta merge is visible only to users signed in as admin or operator. *(Backend, security)*
- **Alert webhook no longer follows redirects** — On a redirect, the webhook secret went along to the other host; a redirect now counts as a failure. **A receiver that redirects (including http to https) gets no alerts after the update — change the URL to the direct target.** An SMTP port outside 1–65535 is rejected on save instead of being silently altered when sending. *(Backend, security)*
- **Sign-in and password** — After changing your own password, the current session stays signed in; only the other sessions end. Reloading the page during a short backend outage lands on the target page instead of the login. Signed-out sessions stay blocked until their token expires, a sign-in attempt with an unknown name takes as long as with a known one, and an admin's two-factor reset ends the account's sessions. *(Backend, management interface, security)*
- **Tasks: search by device and task name** — The search in the task list asks the server and also finds tasks without a device, such as data backups. The type column and type filter know all real task types, "last changed" follows every status change, and the initiator of a command is always the signed-in account. *(Backend, management interface)*
- **Reports: lifecycle separates "Next expirations" and "Overdue"** — Expired warranties no longer push out the upcoming dates; they are listed in a table of their own. "Chronically offline" and the Prometheus metrics no longer count devices in storage, the Compliance report survives absurd warranty periods, and the VPN report shows a message with "Reload" when loading fails. *(Backend, management interface)*
- **System restore ends open operations from the backup** — Deployments (including scheduled ones), captures and rollouts that were still running in the backup are ended after the restore with a visible reason; devices leave the "cloning" state and the NFS shares are removed. A failed restore cleans up its unpack directory, and a manually placed backup file with special characters in its name no longer crashes the backup overview. *(Backend)*
- **Updates: retries per intermediate step, "Confirmed" as the device's report** — In update chains, the number of allowed download retries applies per intermediate step instead of to the whole chain. "Confirmed" carries the addition "reported by the device", because the server cannot verify that the update was applied. Deleting an assignment asks first, and the progress bar uses the delta the device actually downloads. *(Backend, management interface)*
- **Vulnerability scan: more accurate numbers, fixed scanner versions** — The list in the panel is cut to 200 entries only after VEX and KEV assessment and by relevance, the report additionally gives the number of unique findings, and a single broken hit no longer drops the whole image. The scanners syft and grype are built into the backend with a fixed version and a verified checksum; newer ones arrive only through dependency maintenance. The KEV catalogue and the acceptance list are fetched only over HTTPS and with a size limit, and a crash in the evaluation no longer blocks further scans. *(Backend, security)*
- **Cloning and deployments: clear errors instead of silent hangs** — If the NFS share fails, a deployment does not start in the first place and states the reason; database errors appear as such instead of "already active" or "device unknown". Cancelling a deployment leaves finished devices online, an aborted capture or restore run stays "aborted", and a clone without a readable version cannot be removed through the guided delete. Half ISO files and the BitTorrent cache no longer stay behind, and a BitTorrent deployment waits at most 15 seconds for the seeder. *(Backend, management interface)*
- **Setup wizard** — For small rollout networks (/29 and smaller) the wizard no longer suggests an inverted DHCP range and checks that start and end lie within the network. The management network card and DNS servers are preselected, the certificate names include the addresses of the network cards, and names with a comma, slash or plus are rejected. A failed Clonezilla download can be restarted, and the restart after completion waits until the agent is re-signed. *(Backend, management interface)*
- **Interface: messages match the result** — Bulk delete and bulk token re-issue report partial failures with the affected devices, copy buttons report success only when something was copied, and managed certificates are deleted only after confirmation. Where raw translation keys used to appear, there is text; the language button shows the displayed language, and the rollback "defective" checkbox is set again for every job. Tabs follow the back button, a full browser storage no longer breaks any action, and the unreachable "Assign Clone" dialog is gone from the groups view. *(Management interface)*
- **Settings: "Users" tab only for admins** — Viewers and operators got "access denied" when opening the settings; they now land on "General". A user name or e-mail address that is already taken is rejected with a clear message instead of a database error. *(Backend, management interface)*
- **VPN: "Release connection", groups and import** — "Release connection" takes the shortcut only when the server's VPN service has provably never reached the instance: a rejected sign-in is disabled permanently, one that never signed in is released without the 15-second wait. Renaming a group no longer empties it when its member list cannot be read, deactivating a device's VPN also removes its second peer, and the import accepts files only up to 8 MB and does not take over implausible IDs. A management address without TLS (http://) is shown as a plain-text risk, and an unused sign-in key of the server no longer stays behind in the instance. *(Backend, management interface, VPN)*
- **VPN: DNS names of exposures take effect immediately** — The local DNS zones are kept in a file of their own in the data folder, which the DNS service re-reads without a restart: new names are answered locally at once (no 10-second hang without internet), deleted ones disappear at once. The DNS page shows the file as a third preview, and simultaneous changes to exposures no longer remove each other's entries. **On the first start after the update, the DNS/DHCP service restarts once (short interruption).** *(Backend, management interface, VPN)*
- **VPN: server sign-in and reconciliation check more closely** — The server's sign-in requires its address in the local network up front and reports a revoked token, instance outages and a withdrawn ready stamp with their cause instead of a misleading message. Reconciliation adopts default groups renamed in the dashboard, leaves the system group "All" alone and stops working on the old instance after "Release connection"; taking over an instance reports rules disabled there. Exposure ports are accepted only as a plain number from 1 to 65535, in the wizard as well. *(Backend, management interface, VPN)*
- **Agent: rollback cancel works, private mount points, honest VPN state (new agent required)** — A cancelled rollback no longer runs anyway on prepared devices, and no second rollback follows a completed one. When applying updates, the file system is no longer mounted in a directory readable by everyone; self-update, delta download and certificate sync check sizes, addresses and leftovers more strictly. VPN devices count as "active" only with a connected tunnel, a rejected key ends the sign-in loop (chip "Key rejected"), and an incomplete teardown is reported (chip "Teardown incomplete"). *(Agent, VPN)*
- **Database, worker and migrator** — Capture jobs can only have one of the five valid states, a dead column is removed, and the table of deployment devices has indexes. The database check at startup no longer issues locking statements when there is nothing to change. The worker has its own, larger connection pool (24 connections, configurable) with the same time limits as the backend. A long error text with an umlaut used to stop all scheduled tasks until the next restart; it is now truncated correctly. *(Backend, worker)*
- **Scripts and containers** — The update and rebuild scripts keep a custom filter of the Services panel, and it now also takes effect on release servers. The time service chrony no longer answers requests before setup, the control interface of the BitTorrent seeder is reachable only by root, and the NFS container stops immediately. The rebuild continues without GitHub access, a failed Semaphore reset no longer prints a password, and the partitioning on the device requires 22 instead of 21 GB. *(Server services, deploy repo, Tools-ISO, security)*
- **Update notes** — The images for backend, worker and interface as well as for the time service, BitTorrent seeder, NFS server and multicast sender are rebuilt; the DNS image stays. The agent changes take effect only with the next agent release, the installer changes with a rebuilt Tools-ISO; backend and worker adapt the database themselves on startup. **Check beforehand: enter webhook URLs without a redirect and switch your own scripts from the token in the URL to the Authorization header.** *(Backend, worker, Agent, deploy repo)*

## 2026-09-15

- **Redis requires a password, and a stalled Redis no longer freezes the UI** — The internal queue was reachable without authentication, and when it stalled, sign-in and every page hung. Redis is now password-protected (`REDIS_PASSWORD`, added to `.env` automatically by the scripts), all short calls have a 5-second deadline, and the worker only runs jobs that are registered in the database. Sign-out, password reset and data backup now report Redis errors instead of hiding them; a reset link is only used up once the new password is saved. **The update recreates redis, backend and worker; Redis content is lost in the process, signed-in users stay signed in.** *(Backend, worker, deploy repo, security)*
- **Audit log: sign-in floods are combined, retention is configurable** — Failed sign-in attempts are combined into one entry per minute and source with an attempt counter instead of writing hundreds of thousands of rows a day during a flood. The worker deletes old entries after a configurable period (Settings → General, default 365 days). **The first start after the update deletes entries older than 365 days.** *(Backend, worker, admin UI)*
- **Schedules use the configured time zone** — Task templates, recurring maintenance windows, warranty day boundaries, report buckets and date inputs used UTC or a fixed Berlin time. They now follow the time zone under Settings → General → Time Server (NTP), and the field for it is back. **Existing templates therefore run at the configured local time, two hours earlier than before in summer.** *(Backend, worker, admin UI)*
- **The database migrator starts even with an incomplete schema** — If an existing database lacked a table or type, startup aborted and backend and worker restarted in a loop. The migrator now skips the step and names the gap in the log. *(Backend, worker)*
- **Tools ISO and agent download only use the built, signed agent** — When the built agent binary was missing, ISO build and download fell back to an outdated, unsigned binary in the source tree; it has been removed. ISO build and VM start now verify the signature, and device setup verifies it before installing. **After an update on release servers, click "Sign" under Clients → Agent, otherwise ISO build and the cloning VM will not start.** *(Backend, Tools ISO, security)*

## 2026-09-11

- **Capturing a running device now actually starts** — The heartbeat used to strip the capture configuration 300 ms after it was written and queued a new post-clone task on every heartbeat; the capture never began. The device now stays armed until it reboots, and with "Reboot device now" (default on) ThinForge triggers that reboot itself. *(Backend, Admin UI)*
- **An immediate clone deployment reboots the devices itself** — A deployment without a schedule stayed "pending" until someone rebooted the target devices by hand. Every target that has reported recently now receives a reboot task (for BitTorrent only once the torrents are ready); the "Reboot devices now" switch in the create dialog turns this off, and each device shows "reboot sent". Powered-off devices stay armed and start on their next boot. **API: new field `reboot_now` (default `true`) in `POST /clone-deployments` and `POST /capture-jobs`.** *(Backend, Admin UI)*
- **Network settings: the rollout network's prefix length is preserved** — Saving the DHCP settings or the network split discarded the prefix length (and the management DNS servers); time synchronisation then fell back to /24. *(Backend)*
- **Audit trail for clones, clone deployments, reboots, VPN and backups** — 36 actions left no entry in the audit log until now: creating, deleting, restoring, exporting and importing clones; creating, cancelling, restarting and deleting clone deployments; rebooting, shutting down and waking devices (single and bulk); activating, deactivating and re-enrolling VPN; exposures; VPN management (saving and clearing the connection, bootstrap, first connect); creating, restoring and deleting backups; creating, launching and cancelling rollouts. Each now records the operator, the sender address and the target, and the action filter of the logs view knows the new names. *(Backend, Admin UI, Security)*
- **System test suite** — Alongside the API suite there is now a suite that exercises the stack on a test bench with real devices (clone, deployment, rollout, capture, remote desktop, SSH, agent logs, VPN off/on, backup); every intrusive stage sits behind its own opt-in flag. Its first full run found the four findings above. *(Scripts)*

## 2026-09-10 (round 3)

- **The 214 medium-severity findings of the 2 September full review are done** — Eleven tasks by area; each finding re-checked against the current code, then fixed or documented as a decision. *(Backend, agent, scripts, admin UI)*
- **Device master data can be cleared again** — A cleared field (room, user, inventory number, purchase date, warranty, group) silently kept its old value on save. *(Backend, admin UI)*
- **Admin UI: failures are no longer reported as success** — Failed dnsmasq restart, refused clone deletion, settings that could not be loaded (save is locked), abandoned status polling, expired TLS certificates (were shown as valid), unavailable compliance report. Delta export and alert test buttons work again; groups from the third level down are visible; merge deltas only with confirmation. *(Admin UI)*
- **Security: session blacklist fail-closed, audit trail with sender, security tabs admin-only** — During a Redis outage revoked sessions were valid again; the audit trail never said where an action came from and its filter found nothing; viewers could fetch vulnerability and SBOM data. A machine on the rollout network could disarm someone else's rollout through the done callback. Certificate groups follow the device identity, not the reported MAC. *(Backend)*
- **VPN: one DNS name per exposure, bootstrap and "release connection" exclude each other, one time budget** — Two exposures with the same name stole each other's entry; operations keep running server-side when the browser times out; managed server names survive an address change; stably connected devices counted as "unknown" after three minutes. *(Backend, VPN)*
- **Services: no more silent failures in Caddy, Docker, dnsmasq, backup, scan** — Caddy config written atomically and validated; a failed Docker query no longer counts as "not running"; data backup checks free space; restore no longer destroys the target before success; e-mail no longer falls back to plaintext silently (**a channel with credentials and no TLS now needs `allow_insecure_auth`**); retention values with limits instead of a crash; vulnerability scan with timeouts and an honest denominator. *(Backend)*
- **Cloning and updates: staging instead of overwriting, timeouts for every child process, no deleting clones with a running deployment** — ISO download and Tools ISO build only replace the target after success; delta export streams; uploads no longer buffer twice. *(Backend)*
- **Agent: update cleanup protects the snapshot it needs, rollback picks the right version, downloads check space and size** — Also: no self-update during the reboot countdown, no deadlock while applying, a durable marker after a failed update, certificate changes only move forward. *(Agent)*
- **Scripts and Docker: build context without storage and keys, cleanup without losing images, deploy channel from .env** — Plus installer hardening (no NOPASSWD leftovers, SSH password login off, grub-btrfs pinned) and the cloning VM no longer overwrites the Tools ISO. *(Deploy repo, Tools ISO)*
- **Worker and key files: no silent fresh start, interrupted jobs are closed** — An unreadable or empty key file used to trigger a silent regeneration (every session and seal invalid); now startup aborts. The worker closes jobs a restart tore off, restarts crashed schedules and reports devices as really offline. Groups with managed certificates can only be deleted after confirmation; device import, maintenance windows, network and wizard input are checked before anything is written. *(Backend)*
- **VPN UI: operations survive the browser timeout, "Clean up" needs its own confirmation, no lost update when toggling** — Reconcile, exposures and cleanup finish server-side and are read back; cleaning up your own fleet needs a named confirmation; an exposure toggle no longer overwrites someone else's changes; the VPN task table is populated again; CSV export without formula execution. Agent: setup key no longer on the command line, no SSO hang on reconnect, crash-safe pending file, instance switch survives a certificate rotation. *(Admin UI, agent, VPN)*

## 2026-09-05 (round 2)

- **Device heartbeat: address and VPN binding are no longer taken on the device's word** — A device (or anyone holding its heartbeat token) could name any address as its SSH target and bind itself to a VPN peer, occupying licence seats and reviving deactivated devices. The origin observed by the server now decides, and only the server writes the VPN binding.
- **VPN: switching the instance only via "Release connection"** — A different NetBird address could be entered while the ledger, exposures and the server tunnel still belonged to the old instance; reconciliation then failed every minute. The switch is now refused while the installation is bound. The setup wizard no longer releases a working connection on cancel after a short backend outage.
- **Clone deployments: failed starts disarm devices, restarts are guarded, multicast no longer waits forever** — A failed deployment start left devices armed and imaged them on the next boot; a restart could arm devices already in another deployment; a single device that never reported ready blocked all others indefinitely. All three fixed; the maximum wait now applies.
- **Clone scripts: no data loss on a too-small disk or a missing image, callback secret no longer exposed on the network** — With a too-small target disk the partition table was wiped despite the abort; a missing image was reported as success. The device's callback secret was readable by the whole rollout network and is now handed only to the right device at boot time (the backend must be reachable then).
- **Updates: download slots with a cap, one open job per device, agent version published only after signing** — The agent build script now deletes and publishes its outputs through Docker because the output directory is owned by root. A few silent devices could hold every download slot forever; duplicate update jobs per device landed on the wrong one; two agent uploads could merge into one file; the fleet fetched a new agent version whose signature did not match yet.
- **The server now verifies the device's SSH host key** — Until now the server accepted anyone when reaching a device (reboot, terminal, agent update); whoever took over a device's address on the network received the fleet's credentials. The key is remembered on first contact and checked afterwards; it is relearned after reinstallation, an enrollment reset or a version change.
- **Administration: no locking out the last administrator, backup without endless loops, group rollouts only with a group** — One's own account could be turned into a viewer or deactivated, a link cycle in the clone directory took the backup down, and a group-scoped rollout without a group would have re-imaged the entire fleet. All three refused or fixed; a group with a running rollout cannot be deleted.
- **The Tools ISO snapshot tool only offers agent snapshots** — The tool also listed system areas such as `@home` for deletion.
- **Agent: the heartbeat token never goes to an unverified endpoint, updates never leave a device without a root** — On a certificate change the agent sent its heartbeat including the token to an unverified address; a failed snapshot during an update could leave a device without a root file system; a half-received update counted as complete; a missing verification tool was reported as tampering; a server address without https was accepted. All fixed, effective with the new agent.
- **Test suite** — `scripts/e2e/` checks a running system from the outside for the effect of these fixes. Negative tests only use throwaway accounts; the VPN connection settings are saved before the check and written back afterwards.

## 2026-09-05

- **Device enrollment: the install token could do too much** — A heartbeat carrying the install token baked into every clone image could write the record of a not-yet-enrolled device and even fetch its VPN activation key, although the enrollment itself had been refused. An install-token heartbeat may now only enroll; if that is refused, the call changes nothing and returns nothing. The reason for the refusal is shown as a hint on the device status. Over the VPN the request must come from the bound device itself, and installations with an external DHCP server (proxy mode) now enroll devices at all.

- **VPN: a connection test during a short outage confused the reconcile** — If "Test connection" failed while the NetBird instance was restarting, the server remembered that the instance had no networks and then recreated network, resources and router every minute. A failed test is now only shown as an error and no longer overwrites the last valid result; an unsafe reconcile is aborted instead of executed.

- **Login: second factor protected against guessing, lockout only hits the source** — A wrong code cost nothing, and a used code stayed valid for a short while. After five failed attempts the second factor is locked for 15 minutes, a code is valid only once, and failed attempts appear in the audit log. In addition, a single machine flooding the login endpoint used to lock out all operators, including those already logged in; the limit now applies per source, and session renewal has its own generous allowance.

- **Clone import and delta import hardened** — An uploaded clone archive with crafted file names could execute commands on the server when deltas were merged; such archives are now rejected. Imported delta updates used to adopt the signature of another installation and counted as released immediately; they are now re-signed with the local key and released through a rollout only.

- **Rollback by group selected the wrong devices** — When selecting by group, the server took the devices already on the target version and marked exactly that version as defective. The devices that need rolling back are now selected, and a task containing devices on the target version is rejected. Only affected direct API calls, not the interface.

- **Deleted group left its task template running fleet-wide** — When a group was deleted, a task template bound to it stayed enabled and ran on every device without a group from then on. A group with templates can now only be deleted with "force", which disables the templates; already orphaned templates are disabled on the next start.

- **Multicast deployment reported aborted transfers as success** — If the multicast stream broke in the middle of a partition, the device still reported success and the server recorded the new image. Every stage of the transfer is now checked, and an abort is reported with the failing stage.

- **Further hardening** — The agent can no longer delete the currently booted system generation at startup (takes effect with the next agent update). Two BitTorrent deployments of the same clone started at the same time no longer corrupt the distribution cache. SSH access to the cloning VM is reachable from the server itself only, no longer from the network.

## 2026-09-02

- **"Disconnect" and "Cancel" in the VPN wizard hung on a fresh installation** — If setup had already failed in step 1 (for example on an access token the instance rejects), the server's VPN service had never been enrolled. "Disconnect" and cancelling the wizard nevertheless tried to shut it down — and the service then waited for a browser login that could never come. The interface gave up after 30 seconds, the credentials stayed stored, and the VPN page showed a half-configured connection that never existed. Disconnecting no longer waits for that login; after a short deadline it checks directly whether the shutdown took effect. Every connection attempt by the server is time-limited, and the interface waits long enough. *(backend, management interface)*

- **Revoked device credentials could remain valid** — When an old device token was revoked, the interface always reported the operation as done — even when writing to disk failed. In that case the token stayed valid until its transition period expired, and nobody noticed. The same weakness affected token rotation from the other side: if writing failed there, the old token lost its transition period and devices would have been locked out immediately instead of migrating gently. Both operations now report a failure as such, and rotation aborts before the old credentials are removed. *(security, device management)*

- **Bulk actions hid devices that failed** — When restarting or shutting down several devices at once, the server reported "3 of 5 completed" without saying which two were not reached, or why. The response now lists the affected devices individually. *(device management)*

- **The "stop the VM first" hint never appeared** — Creating the partition layout of a cloning base disk requires the cloning VM to be stopped. If it was still running, the operator got an unhelpful error with a reference number instead of that hint — the server was checking for a German message text that never existed. The hint now appears. *(cloning)*

- **Clearer error messages in several places** — The VPN impact analysis run before a deployment reported a database outage as a bad request, and passed internal database details back to the caller; both are fixed. A task template referencing an unknown group produced a generic server error instead of stating that the group does not exist. And deleting a delta image reported every failure as a name conflict. *(backend, deployment, tasks)*

## 2026-09-01

- **Libraries and containers updated** — Database, cache and reverse proxy now run on fresh images, and the management interface and server services on updated libraries. This also includes newer versions of the virtualisation and SSH tools used for cloning and remote maintenance. *(Backend, management interface, server services)*

## 2026-08-31

- **Management API no longer reachable from the rollout network** — The unencrypted port that devices use while cloning over the network previously exposed the entire management API, including sign-in. Only the six status calls that cloning devices actually send (such as "cloning started" and "cloning finished") are now allowed there; everything else is redirected to the encrypted connection, where it remains blocked from the rollout network. Cloning itself runs unchanged. *(server services, security)*

- **Internal status reports of the clone distribution sealed off from the outside** — Nine reporting paths that the distribution services use to report a clone job's progress and errors to the server internally could also be reached from the network without any sign-in; a device on the network could have fed false states into a running job. These reporting paths run entirely inside the server and are now blocked on every network entry point. The cloning process itself is unaffected. *(cloning, security)*

- **Device list now only retrievable with a key** — Two query paths through which an internal helper program obtains the complete list of managed devices (names, network addresses, groups) could be fetched from the network without any sign-in: the intended key check only applied when a key was set up — and in the delivered state none was. Retrieval now always requires a valid key; the server creates it itself on first start and stores it for the helper program. In addition, both query paths are blocked on every network entry point — the helper program operates entirely inside the server. *(server services, security)*

- **User roles now apply to every writing action** — Accounts with the "Viewer" role are meant to be read-only, but could previously trigger 57 writing actions — such as restarting or shutting down devices, creating and starting deployments, or changing network settings. That is closed: viewers can now only read, everywhere. At the same time, 16 system-level actions — starting/stopping/restarting services as well as DHCP, DNS, PXE, network and multicast settings — are now reserved for administrators: accounts with the "Operator" role will receive a permission notice there, while day-to-day operation (devices, groups, images, captures, deployments) remains available to them unchanged. Every role still manages its own account's password and two-factor sign-in. A check in the test suite now ensures that no writing action can be added without a role check. *(management interface, security)*
- **Further hardening of the management interface** — System metrics (device names, MAC addresses, load figures) can no longer be retrieved without signing in. The interface now sends additional protective headers to the browser, and the sign-in endpoint limits how much text a failed attempt can write into the audit trail. *(Backend, server services)*

## 2026-08-29

- **Weakness in the web server's SSH library reviewed and deliberately carried** — Several bundled programs contain a Go library with a server-side SSH weakness. A corrected version has been published, but no web server release yet includes it — the gap currently cannot be closed by anyone. It is not reachable here: no container runs an SSH server, and in the web server program the affected code is verifiably not present at all. The point is documented in the public vulnerability list and will be reviewed again with the next web server release. *(server services, security)*

- **The cloning service no longer asks the router for anything and no longer broadcasts on the network** — The service that distributes images to many devices at once used to start four network services that have no purpose on an isolated distribution network: two router port-forwarding protocols, a local broadcast announcement, and participation in a global lookup network. All four are now off and can be switched back on individually if ever needed. Image distribution itself is unaffected — the direct exchange between devices was already active before and stays that way. *(cloning, security)*

- **Known weakness in the file sharing service reviewed and deliberately carried** — The bundled NFS service contains an open weakness that lets a client widen the share permissions it was granted when mounting. No corrected build exists in the system base currently in use, not even in its development branch. Because ThinForge runs on the local network only and shares are granted exclusively to individual known devices, the point is carried deliberately rather than fixed; it is documented in the public vulnerability list. Worth noting: no automated scan reports this finding, because the corresponding advisory carries no system-base mapping — it was found by hand during an update survey. *(server services, security)*

- **All images are published in the registry again** — After the previous day's security rebuild, seven of the ten ThinForge images had not been published at all, and two more were there in a build predating the rebuild. All ten are now published and were verified one by one. A fresh installation therefore pulls exactly the state that was tested here. *(delivery)*

- **Outdated agent program in the tools medium replaced** — An agent build from April, fifteen versions behind the current one, was still present in the project files. It served only as a fallback and was never used in normal operation, but it shipped with the project files. It has been replaced with the current build. *(agent, delivery)*

## 2026-08-28

- **Background jobs were left sitting after a cache restart** — When the cache service is restarted, for instance during an update, the background service lost its connection to it and never rebuilt it. It processed no further jobs at all — tasks such as joining devices to the VPN, collecting inventory data, or creating a backup simply stopped. The service still looked healthy in the service overview throughout, and only a restart helped. The connection is now re-established on its own; a cache restart now costs only the single job that happened to hit it. *(server services)*

- **Security update for the bundled encryption library** — Seven ThinForge images still carried an older OpenSSL release for which fixes have since been published. All seven were rebuilt and now carry the current release; the corresponding security findings are gone entirely for every image we build ourselves. The security report consequently reads "no open items" again instead of a warning. *(server services, security)*

- **Database and cache brought up to date** — Both base services were moved to the latest release of their images. The program versions themselves are unchanged; what was updated are the bundled system packages. The database's data is unaffected. Sessions signed in to the interface were signed out once afterwards — the cache deliberately holds sessions only transiently. *(server services)*

## 2026-08-27

- **During a system update, a device temporarily stopped reporting its state** — While a device was downloading an update, and afterwards while it waited for an answer to the restart prompt, it sent only a minimal sign of life: load, restore points and VPN state stayed at their previous values in the overview — with the restart prompt open, for up to a quarter of an hour. During that time the device's log filled with the entry "SSH authorized_keys rotated" every few seconds, even though the stored key never changed. Both are fixed: the device now reports its full state during an update as well, and the restart prompt no longer holds it back. *(agent)*

- **The "via relay" note in the VPN device list comes without an explanation** — When a device's connection runs through the relay server, the "Connection" column still says so. A longer explanation used to sit behind that note and opened on hover. The note itself stays exactly as it was; the explanation is gone. *(management interface)*

- **The VM console now appears only once it is genuinely ready** — The console view could previously open while the screen transfer was still starting, showing an error page instead of the screen. It now waits for actual readiness and displays a notice until then. If the transfer stops responding mid-session, a notice explaining the next step takes its place, instead of a dead image. *(clone creation)*

- **The "recreate disk" route now reaches its goal** — When the configured disk size differed from the existing one, a prompt offered to delete the disk and recreate it. It was deleted but not recreated: the start that followed failed, because the disk can only be created through the setup step. The route now leads straight there, pre-filled with the requested size. *(clone creation)*

- **When the cloning VM fails to start, the reason is now shown** — Failed starts reported "Internal error" with no indication of the cause. The reason is now named: if the base disk is missing, the message points at the button that creates it; if there is not enough memory, it states the required and the available amount. The base disk is also checked before the VM is started, rather than letting it start and fail within its first second. *(clone creation)*

## 2026-08-26

- **The cloning VM shut itself down shortly after starting** — Since the cloning VM's console was restricted to the server itself on 25 August, the virtual machine used to create clones switched off again about half a minute after starting. The screen transfer worked perfectly throughout — only the server's start-up check still looked for it by the old route, found nothing there, and judged the running VM to be broken. The check now looks in the right place and the VM stays up. *(clone creation)*

- **Released devices were reachable beyond what was configured** — When you release a device from the client network in the VPN section, you define who may reach it on which ports. Two ports bypassed that: every released device was additionally reachable by **all** VPN devices on ports 443 (HTTPS) and 53 (DNS), regardless of the configured group and ports. For devices with their own web interface — printers, network storage, switches — this left their administration open to everyone. Releases are now kept separately and open only what was configured. Existing releases are migrated automatically at the next reconciliation; nothing needs to be done. *(VPN, management interface)*

- **An altered VPN backup file could open access nobody had configured** — When importing a VPN backup, the releases it contained were accepted unchecked. An edited file could therefore release targets far outside the client network, up to the entire network traffic, and afterwards appeared as an ordinary release. Imported backups are now validated exactly like a manually created release; if any entry does not fit, the file is rejected in full and nothing is changed. *(VPN)*

- **A single device could retrieve the fleet's shared enrolment secret** — A device that had already enrolled could retrieve the shared token used by new devices for their initial enrolment. A stolen credential therefore stayed usable indefinitely, because rotating that token had no effect. Retrieval is now limited to the initial enrolment path. Nothing changes for the devices — they have always renewed their own credential by a different route. *(server services, agent)*

## 2026-08-25

- **The security report shows again how reachable a service is** — The "Exposure" column in the vulnerability overview stayed empty. More than a display was missing: on servers operated from the registry, the container images actually running were never examined, and in the end the report was no longer produced at all. Both are fixed. The report now examines exactly the images running on that server, regardless of whether it is operated on the test or the release channel. A scan that could not examine something now says so explicitly instead of quietly reporting "clean". *(management interface, backend)*

- **The cloning VM's console was reachable across the network** — The screen transfer of the virtual machine used when creating clones was reachable without credentials from any device on the network, while the VM itself runs with elevated privileges. It is now restricted to the server itself and reachable only through the management interface. *(clone creation)*

- **Time synchronisation is correctly classified as a LAN-only service** — The time service was listed in the security overview as reachable over the VPN. It answers requests from the client network only. The classification has been corrected; the service's behaviour is unchanged. *(management interface)*

- **Bundled source code matches the shipped programs again** — ThinForge ships the corresponding source code for the GPL components it contains. Updates to the system base had left the bundled archives stale, no longer matching the program versions actually included. Matching source is now present for every version ever shipped, with checksums and a mapping of which file belongs to which release. *(distribution)*

- **Bundled third-party components updated** — Libraries and system packages across all server images were brought up to date. Functionality is unchanged. This surfaced that the images had not been picking up system package updates at all: twelve vulnerabilities previously classified as "not fixable" had in fact long been fixed and are now gone. The security report lists 50 findings fewer. *(server services, management interface)*

## 2026-08-24

- **Security reporting path documented** — A `security.txt` per RFC 9116 and a PGP key for encrypted reports are now in place, along with documentation of the update path and support duration for security updates. *(server services, management interface, Agent, Tools-ISO)*
...
- **VPN wording: "ThinVPN" and "VPN instance" are now kept apart** — The same term used to denote two different things: "ThinVPN server" meant either the VPN service on the ThinForge server or the remote VPN instance it registers with. The service is still called **ThinVPN**; the remote side is now consistently the **VPN instance** or **VPN management**. A sentence like "register the ThinForge server with the ThinVPN server" no longer occurs. Two older inconsistencies surfaced along the way and are fixed: the no-VPN start page and the status bar named the same state differently, and two release hints spoke of the "NetBird service" instead of the ThinVPN service. *(Management interface)*

- **The server is taken back rather than replaced when it re-registers** — When the server re-registered after losing its connection, ThinForge always created a new entry on the VPN instance; the previous one stayed behind there, with valid access but no assignment to a device. This mainly affected the "Adopt" path. ThinForge now first tries to take the previous entry back — which works as long as it is the same machine, and then no second one appears. If it is a different machine, a fresh registration still happens, and the replaced entry is removed in the process. *(Backend)*

- **The VPN service's autostart block was unreliable in both directions** — On releasing the connection the server's VPN service is meant to be shut down permanently so it does not, after a restart, end up on its own in an instance it no longer manages. That block, however, was neither set reliably nor ever lifted again. It was set only if the service happened to be stopped already at the time of release — normally it was running, and the block never took. Once set, on the other hand, it stayed set forever, even after a successful re-registration. Depending on the history, the server could therefore return unexpectedly after a restart, or drop out of the VPN unnoticed. Both halves are reliable now: shutting down enforces the block, and every re-registration lifts it. *(Backend)*

## 2026-08-22

- **Adopting an existing VPN inventory is switched off for now** — Connecting to a VPN instance that already holds objects used to offer adopting them instead of cleaning up. In doing so the server does not update its own entry on the instance: the old entry stays behind, so the adopted inventory is not the one the server goes on to manage. The button is therefore disabled and states the reason on the spot. Until further notice the reliable path is "Clean up" followed by a rebuild. *(Management interface)*

- **"Release connection": three edge cases secured** — If the decision fell exactly while a reconciliation was running in the background, that run could afterwards recreate mappings to the VPN instance just given up — with the same endless reconciliation as a result. Releasing now waits for a running reconciliation, and a reconciliation aborts if the connection was taken away from it in the meantime. Second, the DNS entries of the removed exposures are cleared in a single pass instead of one at a time — with many exposures that saves just as many restarts of the DNS service, and the feedback now names exactly those entries that really disappeared. Third, the operation survives a crash between saving and following up without leaving DNS entries behind that nobody can find any more. *(Backend)*

- **Cleaning up without full information about the instance removed too much** — If the VPN instance briefly answered the capability check with an error, it counted as "does not know networks" — and the clean-up afterwards would then have removed the mappings to networks, exposures and routers even though those objects were still standing there. Only what the instance actually reported on is cleaned up now. *(Backend)*

- **After reconnecting to the VPN instance, reconciliation never settled** — Anyone who had released the connection to the VPN management and then taken it up again got a reconciliation that caught on the same objects minute after minute and never finished. The reason was in the database: ThinForge records, for every object it creates, the identifier it has on the instance — and those notes stayed behind when the connection was released. After reconnecting they pointed at objects that no longer existed there; ThinForge alternated between trying to recreate them (the name was taken) and removing what it found as foreign (it was tied to an access key). **"Release connection" now clears those notes as well — together with this installation's exposures and their rules**, including the matching DNS entries. If you want to keep your exposures, back them up first under "State → Backup". Nothing on the VPN instance itself is touched, and the devices' assignment to their access stays in place too. *(Backend, management interface)*

- **Cleaning up left invisible remnants in the database** — When emptying a VPN instance, ThinForge dropped its note for every object it deleted individually. Some objects are never deleted individually, though: exposures disappear along with their network, and the group managed by the VPN software itself can never be removed. Their notes stayed behind and led to the same endless reconciliation on the next connection. After a clean-up, whatever had no deletion of its own is now removed as well. If the run aborts early, the notes for everything still standing on the instance are kept. *(Backend)*

- **The first connection to a freshly set-up VPN instance did not come about** — Connecting to a still-empty NetBird instance aborted with a protocol message. The cause lay in the instance's own reply: for an empty list — the exposures of a network that has only just been created, for example — it reports "nothing" rather than an empty list, and ThinForge was not prepared for that. Empty lists are now read as what they are. A genuinely malformed reply is still reported and not mistaken for "there is nothing there" — reconciliation would otherwise recreate the entire inventory. *(VPN integration)*

- **Service indicators stayed at the state the page was opened in** — The "active/inactive" chips in the network, PXE and VPN views were loaded once on opening and never again. If a service died during the session, the page kept showing green — in the DHCP view the resulting empty lease table looked like a network problem when the service had simply stopped. The indicators now refresh on their own; in the network and PXE views the refresh button additionally pulls them along right away. *(Management interface)*

- **The device detail page did not refresh** — Status, "last seen" and the live memory and disk figures stayed at the state the page was opened in, with no button to change that. If a device came online meanwhile, it stayed "offline", and terminal and remote support stayed greyed out. The page now reloads by itself and has a refresh button. Entries you have started in the device data are preserved: while anything in the form is unsaved, it is left untouched. *(Management interface)*

- **New device registrations did not appear on their own** — When a thin client registered while the device list was open, it did not show up. The matching refresh button sat inside the approval box, which is not rendered at all when the list is empty — so there was no way to check without reloading the page. Waiting registrations now appear by themselves. *(Management interface)*

- **The backup panel forgot a running backup** — Anyone who switched pages during a data backup and came back found an apparently idle panel: no progress, no cancel button, and the "Create" button active again but only good for an error message. A running backup is now picked back up when the panel opens and shown with its progress. *(Management interface)*

- **Smaller refreshes** — A group's client table now shows online status and VPN count continuously rather than only as of the moment the group was clicked, and has its own refresh button. The warning list for duplicate IP addresses refreshes too, instead of standing still until the network view is left. And the counter at the end of initial setup no longer yanks anyone to the login page who navigates back into the application during those last seconds. *(Management interface)*

- **The dashboard only refreshed half of itself** — The metric tiles at the top kept reloading, the cards below them did not: service status, pending approvals, running and failed tasks, rollouts, compliance and "last seen" all stayed at the state the page was opened in. Because the tiles beside them visibly kept ticking, the whole page looked current — an admin would see "0 failed tasks" indefinitely and miss newly waiting devices. Nor was there a button to put it right. The full page now reloads on the same cycle, and the header carries a refresh button. *(Management interface)*

- **"Refresh" in the reports never showed anything new** — The selected period was pinned when a report tab was opened, end point included. Clicking refresh really did fetch the data again, but kept querying that same old window: tasks, incidents, rollouts or SSH commands added since opening fell outside it and never appeared — in the SSH report this meant the exact same figures came back every time. The period is now held as "the last 7/30/90 days" and resolved against the current time on every load. Eight report tabs were affected. *(Management interface)*

- **The deployment overview froze at the exact moment a deployment finished** — While a deployment was running the overview refreshed by itself; the instant the last one was done, it stopped dead. Yet that is precisely when the server is still working: the closing action on the devices — restart or shut down — and the final per-device rows only follow afterwards. What stayed on screen was therefore the second-to-last state, until someone refreshed by hand. The overview now reloads once more after completion, including the expanded device list, and reports the result — as a warning rather than a success when devices failed. *(Management interface)*

- **The deployment overview sat still after restarting a single device** — Restarting one failed device inside a finished deployment refreshed the per-device rows, but the overview itself did not pick the deployment back up as running. The restarted device's progress then stopped moving and its completion went unreported. The overview now picks the deployment up again. *(Management interface)*

## 2026-08-21

- **Orphaned VPN assignments can now be cleared straight from the warning dialog** — With no connection to the VPN management left, the notice about VPN devices came back on every deployment, because nothing releases those assignments any more. The dialog now offers a checkbox: when ticked, exactly the listed devices are marked as deactivated, release their licence seats and shut their VPN service down at their next heartbeat — the notice no longer appears for them. The box is unticked by default, appears only when the connection has been released, and only for administrators. The matching access on the VPN instance stays in place and has to be removed there separately. *(Backend, management interface)*

- **A deployment warned about VPN devices even though the VPN connection no longer exists** — Creating a deployment or a rollout listed the affected VPN devices and asked for confirmation — including on installations whose connection to the VPN management has been released. The message also promised that ThinForge would issue a fresh access key afterwards; without a connection that is exactly what does not happen. The preview now checks the same thing the deployment itself checks. With a connection in place, nothing changes. Without one, the notice says what actually happens: these devices' access does not survive the re-imaging and has to be set up by hand once the connection is restored. *(Backend, management interface)*

## 2026-08-20

- **The VPN device list now shows when a tunnel runs through the relay server** — Whether a connection comes up directly or is routed through the VPN instance's relay has always been reported by the device, but was visible nowhere. The detour is noticeably slower and was, among other things, why updates crawled for hours on individual devices. Affected devices now carry a marker in the "Connection" column, with an explanation on hover. Direct connections stay unmarked — the marker is meant to stand out, not to appear on every row. *(Management interface)*

- **Removed the "VPN statistics" placeholder from the device list** — An empty box sat permanently below the VPN device list, announcing that an evaluation would follow later. The technical basis for it no longer exists since the VPN connection was rebuilt, so the promise stood there with no prospect of being kept. The box is gone; the per-device details are unchanged. *(Management interface)*

- **A device whose VPN access had been revoked kept trying to connect indefinitely** — Revoking works on the VPN instance: the device's access is deleted there, so it can no longer get in. Until now, though, no message reached the device itself. It kept its worthless credentials, kept the VPN service running, and knocked once a minute at a counterpart that no longer knows it — across reboots, too. In the device list it also stayed on "paused" instead of "not activated", because it still considered itself enrolled. The server now tells every device, in every heartbeat, whether its VPN access is meant to exist. When it goes away, the device shuts the service down and clears the leftover credentials, which puts the display right again. Devices whose access was revoked before this update are included: they are picked up once while the update is applied and shut the service down at their next heartbeat. Revoking itself is unchanged — it still takes effect on the VPN instance immediately, whether or not the device is currently reachable. *(Backend, Agent)*

## 2026-08-17

- **Updates crawled at 1 Mbit over the VPN connection although the line offered far more** — The bandwidth control evaluates the connection's latency variation so it stays out of the way of a parallel session. On connections through the VPN relay the latency varies enough by itself that the control mistook that variation for congestion it had caused: it throttled every two seconds and never recovered — a 450 MB update took hours instead of minutes. The control now subtracts the variation, no longer throttles below its own allowance, and breaks out of a permanent throttle by itself. Yielding to other traffic is unchanged. *(Agent v2.18.0)*

- **VPN clients outside the premises could not resolve the server name** — On installations set up before 2026-08-04 whose network settings had not been saved since, the DNS service was missing one line: it listened only on the device network and discarded every name query arriving through the VPN tunnel — without a message. Affected devices then asked the DNS server at their own location, which does not know the server name, and stopped reporting in although their tunnel was up. The backend now checks the generated configuration against the current one at every start, renews it on a mismatch and restarts the DNS service as part of that. *(Backend)*

- **A freshly set up VPN connection stayed on "not set up"** — After "Register server" the reconciler immediately undid the setup that had just completed: the safeguard that rescues an existing configuration after a database loss already counted the group and the server entry as a configuration worth preserving — yet registration creates both itself. The wizard kept reporting "The initial connection is not complete yet", and the VPN objects were never created. The safeguard now only triggers on objects that were actually applied; after a database loss it works exactly as before. *(Backend)*

- **"Release connection" now also ends the server's own VPN access** — Until now the command only removed the address, access token and probe result. The server itself stayed in the VPN with a running tunnel and kept announcing the routes into its device network — visible nowhere, and no longer stoppable from the interface either. Its own access is now ended first, and for good: the service no longer connects by itself, not even after a restart. Nothing on the VPN instance is touched, as before. If ending it fails, the connection is released regardless — the interface then says the tunnel needs checking by hand. *(Backend, management interface)*

- **The setup wizard no longer skips a server registration that is missing** — When a **second** ThinForge installation adopts the configuration of a first, the server entry recorded in it still belongs to the old server. The wizard jumped straight to the summary anyway: setup counted as finished while not a single tunnel ran on the new machine and the routes into the device network still pointed at the old server. It now leads to the server registration, with the reason stated. *(Backend, management interface)*

- **A migration could open a port nobody had granted** — On an existing installation where two host modules shared a name and differed only in capitalisation, the update carried the older one over as an exposure, as documented. Its **access rules**, however, arrived together with those of the skipped module — even when that module was switched off and pointed at an entirely different machine. A port therefore stood open on a machine nobody had ever granted it for. The skipped module's rules now fall away with it; the log still names it. *(Backend)*

- **Losing the database no longer takes the VPN setup with it** — If ThinForge lost the VPN part of its database while the VPN instance kept running, reconciliation treated the entire configuration as foreign and cleared it within a minute — exposures, network objects and router alike. That configuration is precisely what "Adopt" in the setup wizard needs, the way back after a data loss; it was gone before anyone could open the wizard. Reconciliation now recognises the situation and stops: **nothing is deleted**, the setup state returns to "not configured", and the wizard offers to adopt. Devices come back without re-enrolling, as before. *(Backend)*

- **Reconciliation never settled on migrated installations** — Carried-over host modules kept their target address in the old management's notation; the VPN management returns it in another. Reconciliation therefore saw a permanent discrepancy and rewrote the same exposure every minute — successfully, endlessly, for every carried-over exposure. The "in sync" state was never reached. Addresses are now brought into the form the exposure wizard requires, both when carrying modules over and when restoring a backup. *(Backend)*

- **Restoring a backup now clears DNS names too** — When a backup removed an exposure that still existed here, everything went except its DNS name: the device network kept resolving it to a target nobody had granted any more. The name now falls in the same pass that refreshes the remaining ones — still once for all entries. Names belonging to ThinForge itself remain untouched. *(Backend)*

## 2026-08-15

- **The VPN integration has been rebuilt — a target state instead of individual commands** — ThinForge now describes what the VPN management is supposed to look like and reconciles that state every minute, instead of sending each change over one at a time. Deviations are healed, objects without a counterpart are removed, and every action is recorded with its outcome and failure reason in a log shown by the new "State" tab — alongside "Check" and "Apply now". This brings the most important new operating rule: **nothing is maintained by hand in the VPN dashboard**; objects created there disappear at the next reconcile. *(Backend, admin interface)*

- **Exposures replace the host modules — with a wizard and a preview** — Making a target in the client network reachable over the VPN is now a four-step wizard: target (with an optional DNS name), access rules with presets for RDP, VNC, HTTPS and SSH, a preview of exactly the objects that will be created, and apply with a subsequent verification. Ports are single values — a range such as 3000-4000 is not understood anywhere along the chain. Existing host modules are taken over as exposures automatically on the first start after the update; where two legacy modules share a name and differ only in capitalisation, the older one wins — the one passed over is named in the log and has to be recreated. *(Backend, admin interface)*

- **First connection asks what should happen to an existing inventory** — The setup wizard reads the VPN instance and shows what it finds: empty, this installation's own earlier setup, another installation's, a legacy configuration, or something entirely foreign. If it recognises its own setup, that setup can be **adopted** — the mapping is reconstructed from the inventory, and **no device has to re-enrol**, existing tunnels stay up. That is the way back when the database is lost. It has one limit: the DNS names of exposures are stored nowhere on the instance. Where the exposure still exists here, its name simply stays — it only has to be re-entered when the database was lost as well (or restore an export file, which carries it). Exposures that already exist here — on an existing system the first start after the update takes the previous host modules over — are merged rather than duplicated. Otherwise the wizard shows an inventory and deletes only after explicit confirmation. None of this happens on its own. *(Backend, admin interface)*

- **A device deleted by hand in the VPN dashboard does not heal by itself** — When a device's access is deleted directly in the VPN management, ThinForge shows it as disconnected but deliberately keeps the mapping: after a re-enrolment that mapping is the only pointer to the stale entry still to be cleaned up. The way back is **"Disable VPN" followed by "Enable VPN"** — the device receives a fresh access key in the process. A reimage or a deployment disables the affected devices on its own anyway. *(Backend)*

- **"Disconnect" now only cleans up on this side** — The command removes the address, access token and probe result here; the VPN instance itself is left untouched, and its networks, rules and device access keep working. That is intentional: emptying an instance is a different, far-reaching decision. It is taken in the first-connection wizard, which shows the inventory first and deletes only after confirmation. *(Backend, admin interface)*

- **A second click on "Register server" no longer touches anything** — On a fully set-up installation the step now returns the stored state instead of registering the server a second time — no tearing down the local tunnel, no second access key and no web server reload. Registering again would have left a second server entry behind: valid access, no mapping, and the routes pointing at the wrong identity. To reapply the VPN objects, use "Apply now" in the "State" tab instead. If the server entry is missing from the instance, registration runs in full again. *(Backend)*

- **The VPN setup can be backed up and restored** — The "State" tab exports exposures, rules and device mappings as a file and reads them back in; the access token is deliberately left out, as it is sealed to this installation. Exposures with a DNS name are carried into name resolution after an import — once for all entries, and likewise after an adoption in the setup wizard. Device mappings without a matching device are skipped and appear **only in the log**. In addition, a regular backup of the VPN instance itself (`store.db`) is recommended: it sits outside ThinForge's reach and is the precondition for devices away from the office surviving the loss of that instance without an on-site visit. *(Backend, admin interface)*

- **Write actions in the VPN area require administrator rights** — Enabling, disabling, enabling a whole group, re-enrolling the fleet, creating and editing exposures, checking and applying, export and import, as well as the connection test and inventory check of the instance now consistently require administrator rights. This now covers the first connection too: on an installation that is not yet set up, operators see a notice instead of the wizard that would have answered every step with an error. Operators keep the full read-only view; the remaining controls are hidden or disabled for them rather than failing on click. *(Backend, admin interface)*

- **Devices can be re-enrolled without ever having two tunnels** — When the fleet moves to a different VPN instance, the agent swaps its configuration in one sequence: back up, tear down the old tunnel, register anew, verify — and not just that the tunnel is up, but that it actually carries the server. If a step fails, the device restores the previous configuration itself and reports where it failed; without a viable network the running tunnel is not torn down in the first place. Devices away from the office stay reachable this way. New in agent v2.17.0. *(Agent, Backend)*

- **After the update, run the VPN setup wizard once** — The new VPN integration starts out as “not yet configured” on every installation, including those where the VPN was already running. Until the wizard has been completed once with the VPN management access token — via **Adopt**, where an inventory exists —, **reconciliation rests**: the status bar reports “setup not complete”, devices show their connection state as unknown, and nothing is created and nothing removed. Existing tunnels keep running throughout; ThinForge simply knows nothing about them. Previous host modules have already been taken over as exposures by that point. *(Backend, admin interface)*

## 2026-08-12

- **A long value no longer breaks a device's status report** — Two fields whose length is not set by the server had a fixed upper limit: the agent version and the list of available restore points. Once exceeded, not just the field failed but the entire status report — the device appeared offline while it was running. Both fields are now unbounded. *(Backend)*

- **System groups can no longer be renamed** — The three system-created VPN groups were protected against deletion but not against renaming. A rename would have split the fleet silently: the group is resolved by its name, so it would be recreated empty while the existing members lose access without notice. Both paths are now equally protected. Also: an inventory check that merely looked empty because the query failed no longer counts as an empty instance. *(Backend, Management UI)*

- **The state "configuration is on the device" now actually arrives** — When the VPN configuration is staged on a device but enrolment is still pending, the agent has reported this since v2.16.9. The database rejected that state — and because a signal's VPN report is written in one piece, the entire report was lost with it, including the failure reason: precisely the information explaining why the device is not reaching the VPN. Every ten seconds, permanently. The state is now accepted and the report arrives in full. *(Backend)*

- **Aborted VPN operations no longer leave silent residue** — If registering the server with the VPN service failed, the access key that had already been issued stayed behind unused: it still granted entry, but was recorded nowhere and was therefore never found during cleanup. It is now withdrawn. Likewise, creating a VPN group now reports an error if the ownership record could not be written — without it, a later teardown does not remove the group. *(Backend)*

- **Migrating to a new server now really replaces the predecessor** — When management moves to a new server, it registers with the VPN service under the same name — it is the same role. Until now the old entry stayed alongside it, and routes and rules kept pointing at it: the local network was no longer reachable over the VPN, without anything reporting an error. The superseded entry is now removed and everything remaining is moved over to the new server. If it cannot be removed, the log states what is left to do on the VPN service. *(Backend)*

- **"Unknown" is no longer a disconnection** — While synchronisation with the VPN service is stuck, the server simply knows nothing about individual devices. The list showed that as "disconnected" anyway — so, with a stalled sync, the entire fleet. It now distinguishes three states: connected, disconnected and unknown. *(Admin interface, backend)*

- **A VPN access now provably belongs to exactly one device** — If a device name was renamed and later reused, two devices could point at the same VPN counterpart. Switching one off then removed the other's access. The assignment is now made in two separate steps and enforced as unique in the database; existing double assignments are resolved during the update, and the affected devices re-register on their own. *(Backend)*

- **"Refresh" no longer reports success when nothing was synchronised** — If the VPN service's device list demonstrably did not belong to this server, the sync aborted silently and still reported success with zero devices synchronised. It now states the reason. Releasing the connection without clearing also removes the mirrored VPN addresses — they were no longer reachable from this server but were still used for automated access. *(Backend, admin interface)*

- **Teardown no longer gets stuck on side issues** — Removing the VPN data could be blocked permanently by three things: one of our own groups still referenced in the service's DNS settings, old routes from an earlier model, and a link that can only be released on the service itself. The server now clears the first two; for the third, the message states what to do on the VPN service instead of pointing at a retry that can never succeed. Such legacy routes also now count as existing data when connecting. *(Backend)*

- **An aborted factory reset no longer haunts you** — The list of devices a factory reset is meant to restore used to survive every teardown. If the connection was released instead and a new one set up later, the next factory reset pulled the entire old device list back — including a licence refusal over devices nobody had activated. The list is now given up together with the data; when releasing without clearing, it is deliberately kept. *(Backend)*

- **A device rejoins the VPN after being set up again** — If a device's VPN access was deleted on the service — when reimaging, or after a failed cleanup — the server kept the old assignment. The setup instruction requires that none exists, so the device silently never rejoined the VPN, without an error. Assignments pointing at deleted counterparts are now released, provided the device never confirmed them itself. *(Backend)*

- **Your own additions on the VPN service are preserved** — If descriptions, additional access conditions, further sub-rules, groups or nameservers had been added there, the next reconciliation removed them without a word: it did not know about them and rewrote the objects in full. Descriptions and access conditions are now carried along; objects whose extension cannot be represented here are left untouched and named in the configuration tab. *(Backend)*

- **Switching to a different VPN instance is detected and queried** — If credentials were entered that point to an instance other than the one this server is registered on, it silently set itself up there. The devices of the previous instance kept their access while the server lost the only mapping to them; choosing "delete" even reported success although nothing had been removed. The server now refuses this and explains why. If abandoning the previous instance is confirmed explicitly, the dialog states beforehand what remains behind there. *(Backend, admin interface)*

- **Releasing the VPN connection — with or without clearing the VPN server** — Releasing the connection always removed every object the server had created on the VPN service. For rebuilding a server or moving to another ThinForge that is the wrong move: the devices should stay in the VPN and keep working. The dialog now offers both routes before the confirmation and states the consequence of each; the previous behaviour remains the default. *(Admin interface, backend)*

- **Connecting warns about existing data** — If the VPN service already held data, ThinForge used to settle into it unasked. Before the credentials are saved, an overview of the existing data now appears, together with the devices known here under the same name, and the choice between adopting and deleting. Nothing is saved or changed until that decision; without it the server refuses to connect. *(Admin interface, backend)*

- **Adopted devices are matched to their entries again** — A device's access key is consumed once setup completes, and the VPN connection then persists independently of the server. After a rebuild or a move, such devices could no longer be matched even though they were working in the VPN. They are now reconnected automatically via their device name — only where the name is unambiguous and only if the connection actually exists. *(Backend)*

- **The configuration tab shows foreign devices with their connection state** — Devices on the VPN service that this ThinForge does not manage were listed by name only. Each entry now shows whether the connection exists and whether a device of that name is known here. That distinguishes your own fleet after a move from devices belonging to another installation. *(Admin interface, backend)*

- **VPN devices no longer vanish from the overview** — Removing a group's VPN flag or detaching a device from its group cost it neither VPN access nor its licence seat — it merely disappeared from the overview, and with it the only way to switch it off. Such devices now remain listed, are marked accordingly, and can be deactivated there. The notice shown when deleting a VPN group also promised the opposite of what actually happens and has been corrected. *(Backend, admin interface)*

- **Reconciliation only touches what belongs to it** — A VPN group created through the interface was deleted by the next reconciliation or kept the display permanently on "not in sync"; groups sharing a name could be created and rules bound to the wrong one; an object switched off on the VPN service was either silently switched back on or reported as "in sync" despite being ineffective. Groups are no longer deleted, a second identical name is rejected, and a switched-off object is reported rather than flipped back — including when only the rule inside it is switched off while the object itself still looks active. *(Backend, admin interface)*

- **The connection display states what it actually knows** — An empty or failed response from the VPN service used to be taken as fact and could show the entire fleet as disconnected; an expired access token went unnoticed because the status stayed green regardless. Missing data now counts as "nothing known" rather than "disconnected", the status has its own error state with a reason, and an unreadable access token names both ways out instead of showing a technical message. In addition, a foreign rule that lifts the VPN's access restriction is now reported. *(Backend, admin interface)*

- **VPN setup works on networks without internet access** — Setting up a device checked a public address for reachability beforehand and aborted on isolated networks even though the VPN service was reachable locally. Only the reachability of the VPN service matters now. The setup instruction was also repeated on every check-in, tearing down the connection that had just been established — it is now rate-limited and an already-consumed key is recognised. *(Device agent, backend)*

- **Factory reset and adoption either finish or say why not** — During a factory reset an incidental configuration error blocked completion of the pending list, deleted devices lingered in it as dead entries, and a deliberately switched-off device was set up again unasked. When adopting the server identity, a retry after an abort was no longer possible, and a failed deletion was reported as success. Both are fixed; in addition, "not yet set up" is now a state of its own with a button instead of a green success message. Adopted devices are now included in the factory reset — until now they were missing from its pending list and silently lost access when it ran. *(Backend, admin interface)*

- **Backups keep the activation state of devices** — Creating a system backup stripped the key material of VPN devices — inadvertently along with the record that a device had been activated at all. After a restore, activated devices counted as never set up. That record is now kept; only the key material is removed. *(Backend)*

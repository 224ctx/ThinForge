#!/bin/bash
#
# ThinForge — CachyOS Installation vorbereiten + nachbereiten (XFCE-Edition)
#
# Dieses Script hat zwei Modi:
#
#   VORHER (vor dem Calamares-Installer):
#     sudo bash install-cachyos.sh prepare
...
#     sudo bash install-cachyos.sh finish [--server=https://...]
#     → GRUB mit ThinForge Boot-Logik
#     → ThinForge Agent installieren
#
# Ausfuehrung von der CachyOS-XFCE Live-ISO oder vom eingerichteten System:
#   mount /dev/sr1 /mnt
#   sudo bash /mnt/install-cachyos.sh prepare
#   # → Installer starten, CachyOS installieren (btrfs!)
#   # → Ins neue System booten
#   sudo bash /mnt/install-cachyos.sh finish
#
...
do_prepare() {
    ...
    # Calamares mount.conf — verschiedene Pfade je nach CachyOS-Version
    local mount_conf=""
    for candidate in \
    ...
    echo ""
    echo -e "  1. Start the CachyOS installer (Calamares, XFCE edition)"
    ...
    echo -e "       → Partition 2: remaining space, btrfs, /"
    echo -e "       → Partition 3: already created in UI — do NOT reassign (stays data partition)"
    echo -e "     - Choose btrfs as filesystem!"
    echo -e "     - Bootloader: ${YELLOW}GRUB${NC} (NOT systemd-boot/refind — ThinForge requires GRUB)"
    echo -e "  3. Perform installation"
    ...
    echo -e "     ${CYAN}sudo bash /mnt/install-cachyos.sh finish${NC}"
    echo ""
}
...
do_finish() {
    ...
    if [ ! -d /boot/grub ] && [ ! -f /etc/default/grub ]; then
        fatal "GRUB not found — CachyOS appears to be installed with systemd-boot/refind.\nThinForge requires GRUB for snapshot boot."
    fi
    ...
    # ── GRUB: CachyOS-nativen Bootloader beibehalten ──────────────────
    # Kein eigener GRUB-Bootloader. CachyOS' grub-mkconfig + grub-btrfs
    ...
    log "Configuring GRUB (CachyOS-native + grub-btrfs)..."
    ...
    if [ -f /usr/lib/initcpio/hooks/grub-btrfs-overlayfs ]; then
        ...
        warn "grub-btrfs-overlayfs hook not found — snapshot boot remains read-only"
    fi

    # Snapper / Timeshift deinstallieren (CachyOS bringt teils snapper-tools mit;
...
TSYNC
        ...
        warn "Could not determine NTP server — clone will remain unsynchronized using default NTP pool without internet"
    fi
    ...
    local BRANDING_SCRIPT="${SCRIPT_DIR}/DistroTweaks/install-branding-cachyos.sh"
    if [ ! -f "$BRANDING_SCRIPT" ]; then
        for mnt in /mnt /mnt/iso /run/media/*/THINFORGE_TOOLS /media/*/THINFORGE_TOOLS; do
            if [ -f "${mnt}/DistroTweaks/install-branding-cachyos.sh" ]; then
                BRANDING_SCRIPT="${mnt}/DistroTweaks/install-branding-cachyos.sh"
                break
            fi
        done
    fi
    if [ -f "$BRANDING_SCRIPT" ]; then
        log "Setting ThinForge branding (wallpaper + boot splash)..."
...
case "$ACTION" in
    ...
    finish)
        ...
        ;;
    --help|-h|"")
        echo "ThinForge CachyOS Installation (XFCE edition)"
        echo ""
        echo "Usage:"
        echo "  install-cachyos.sh prepare              Prepare Calamares (before the installer)"
        echo "  install-cachyos.sh finish [--server=URL] Set up GRUB + agent (after the installer)"
        echo ""
        echo "Workflow:"
        echo "  1. Boot from CachyOS-XFCE Live-ISO"
        echo "  2. Mount Tools-ISO:    sudo mount /dev/sr1 /mnt"
        echo "  3. Prepare:            sudo bash /mnt/install-cachyos.sh prepare"
        echo "  4. CachyOS installer: choose btrfs + GRUB!"
        echo "  5. Boot into the new system"
        echo "  6. Mount Tools-ISO:    sudo mount /dev/sr1 /mnt"
        echo "  7. Finish:             sudo bash /mnt/install-cachyos.sh finish"
        ;;
    *)

#!/bin/bash
#
# ThinForge — Manjaro Installation vorbereiten + nachbereiten
#
# Dieses Script hat zwei Modi:
#
#   VORHER (vor dem Calamares-Installer):
#     sudo bash install-manjaro.sh prepare
...
#     sudo bash install-manjaro.sh finish [--server=https://...]
#     → GRUB mit ThinForge Boot-Logik
#     → ThinForge Agent installieren
#
# Ausfuehrung von der Manjaro Live-ISO oder vom eingerichteten System:
#   mount /dev/sr1 /mnt
#   sudo bash /mnt/install-manjaro.sh prepare
#   # → Installer starten, Manjaro installieren
#   # → Ins neue System booten
#   sudo bash /mnt/install-manjaro.sh finish
#
...
do_prepare() {
    ...
    # Calamares mount.conf — verschiedene Pfade je nach Manjaro-Version
    local mount_conf=""
    for candidate in \
        /etc/calamares/modules/mount.conf \
        /usr/share/calamares/modules/mount.conf \
        /run/miso/sfs/livefs/usr/share/calamares/modules/mount.conf; do
        if [ -f "$candidate" ]; then
    ...
    if [ -z "$mount_conf" ]; then
        fatal "Calamares mount.conf not found.\nAre you on a Manjaro Live-ISO with desktop?"
    fi
    ...
    # gehoert zum gleichen Subvolume @root. Defaults von Manjaro/CachyOS
    # legen separate @home, @cache, @log an — die werden vom Btrfs-Send-
    # Delta nicht erfasst. Ergebnis fuer den Operator: Update bringt zwar
    # die Programmdateien, aber Desktop-Verknuepfungen / User-Configs
    # in /home bleiben auf dem alten Stand.
    #
    # Wir ersetzen den gesamten btrfsSubvolumes:-Block durch einen
    # Single-Entry-Block. Indentation orientiert sich an dem in der Datei
    # bereits vorhandenen Stil (4 Spaces, Standard-Calamares).
    local changed=0
    for conf in \
        /etc/calamares/modules/mount.conf \
        ...
        awk '
            ...
            # Block endet sobald wieder ein Top-Level-Key kommt
            in_block && /^[A-Za-z]/ { in_block = 0; print; next }
            in_block { next }
            { print }
        ' "$conf" > "${conf}.tmp" && mv "${conf}.tmp" "$conf"
        log "Updated: $conf (single-root layout, @home/@cache/@log removed)"
        changed=$((changed + 1))
    done
    ...
    echo ""
    echo -e "  1. Start the Manjaro installer (Calamares)"
    echo -e "  2. During partitioning:"
    echo -e "     - Select 'Erase disk' (recommended)"
    ...
    echo -e "     - Choose btrfs as the filesystem!"
    echo -e "  3. Perform the installation"
    echo -e "  4. Boot into the new system (NOT the live ISO)"
    echo -e "  5. Mount the Tools-ISO and run finish:"
    echo -e "     ${CYAN}sudo mount /dev/sr1 /mnt${NC}"
    echo -e "     ${CYAN}sudo bash /mnt/install-manjaro.sh finish${NC}"
    echo ""
}
...
do_finish() {
    ...
    if [ -f /run/miso/bootmnt/manjaro ] || [ -d /run/miso ]; then
    ...
    if echo "$root_source" | grep -q "@root"; then
        ...
        warn "Delta updates still work as long as the subvolume name is consistent."
    else
        warn "No btrfs subvolume detected as root: $root_source"
    fi

    # ── GRUB: Manjaro-nativen Bootloader beibehalten ──────────────────
    # Kein eigener GRUB-Bootloader. Manjaro's grub-mkconfig + grub-btrfs
    ...
    log "Configuring GRUB (Manjaro-native + grub-btrfs)..."
    ...
    if [ -f /usr/lib/initcpio/hooks/grub-btrfs-overlayfs ]; then
        if ! grep -q "grub-btrfs-overlayfs" /etc/mkinitcpio.conf; then
            ...
            log "grub-btrfs-overlayfs already in mkinitcpio.conf"
        fi
    else
        warn "grub-btrfs-overlayfs hook not present — snapshot boot remains read-only"
    fi

    # Timeshift + Autosnap deinstallieren (nicht benoetigt, grub-btrfs reicht)
    local ts_pkgs=()
    pacman -Qi timeshift &>/dev/null && ts_pkgs+=(timeshift)
    pacman -Qi timeshift-autosnap-manjaro &>/dev/null && ts_pkgs+=(timeshift-autosnap-manjaro)
    if [ ${#ts_pkgs[@]} -gt 0 ]; then
        pacman -Rns --noconfirm "${ts_pkgs[@]}" 2>/dev/null || true
        log "${ts_pkgs[*]} uninstalled"
    fi
...
TSYNC
    ...
    local BRANDING_SCRIPT="${SCRIPT_DIR}/DistroTweaks/install-branding-manjaro.sh"
    if [ ! -f "$BRANDING_SCRIPT" ]; then
        for mnt in /mnt /mnt/iso /run/media/*/THINFORGE_TOOLS /media/*/THINFORGE_TOOLS; do
            if [ -f "${mnt}/DistroTweaks/install-branding-manjaro.sh" ]; then
                BRANDING_SCRIPT="${mnt}/DistroTweaks/install-branding-manjaro.sh"
                break
            fi
        done
    fi
...
case "$ACTION" in
    ...
    finish)
        ...
        ;;
    --help|-h|"")
        echo "ThinForge Manjaro Installation"
        echo ""
        echo "Usage:"
        echo "  install-manjaro.sh prepare              Prepare Calamares (before the installer)"
        echo "  install-manjaro.sh finish [--server=URL] Set up GRUB + agent (after the installer)"
        echo ""
        echo "Workflow:"
        echo "  1. Boot from Manjaro Live-ISO"
        echo "  2. Mount Tools-ISO:    sudo mount /dev/sr1 /mnt"
        echo "  3. Prepare:            sudo bash /mnt/install-manjaro.sh prepare"
        echo "  4. Start Manjaro installer and install (choose btrfs!)"
        echo "  5. Boot into the new system"
        echo "  6. Mount Tools-ISO:    sudo mount /dev/sr1 /mnt"
        echo "  7. Finish:             sudo bash /mnt/install-manjaro.sh finish"
        ;;
    *)

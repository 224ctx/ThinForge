#!/bin/bash
#
# ThinForge — Branding (Wallpaper + Boot-Splash) fuer Arch Linux
#
...
#
# Aufruf:
#   sudo bash install-branding-arch.sh              # eigenstaendig
#
# Wird ausserdem automatisch aus install-arch.sh (finish) aufgerufen.
#
...
# Arch: 'plymouth' muss als HOOK in mkinitcpio.conf stehen, sonst startet der
# Splash nicht (gemeinsame Logik in branding-common.sh).
